...
Get_Name (gs_t node)
{
	...
	if (node == NULL) {
		++anon_num;
#ifdef W2C
		sprintf(buf, "_anonymous_%d", anon_num);
#else
...
#ifdef W2C
/* 
 * add by mw, when we want to change the TY_pointed, we
 * should create a new TY entry because TY entry may be
 * shared by other st or wn. First look if there is 
 * already satifactory one 
 */
TY_IDX Create_Ty(TY_IDX origty, TY_IDX pointed) {
  TY_IDX newty = origty;
  TY_IDX ty;

  for (ty = 1; ty < TY_Table_Size(); ty++) {
    if ((TY_size(ty) == TY_size(origty) &&
          TY_mtype(ty) == TY_mtype(origty) &&
          Ty_Table[ty].flags == Ty_Table[origty].flags &&
          Ty_Table[ty].Fld() == Ty_Table[origty].Fld() &&
          Ty_Table[ty].Pointed() == pointed &&
          Ty_Table[ty].name_idx == Ty_Table[origty].name_idx)) {
      break;
    }
  }
  if (ty == TY_Table_Size()) {
    TY_IDX idx;
    New_TY(idx);
    TY_Init(Ty_Table[idx], TY_size(origty), 
            TY_kind(origty), TY_mtype(origty), 
            Ty_Table[origty].name_idx);
    Ty_Table[idx].flags = Ty_Table[origty].flags;
    Ty_Table[idx].Set_fld(Ty_Table[origty].Fld());
    Ty_Table[idx].Set_pointed(pointed);
    Set_TY_IDX_index(newty, TY_IDX_index(idx)); 
  } else {
    Set_TY_IDX_index(newty, ty);
  }
  return newty;
}

/*
 * add by mw, for case that param is a pointer to struct
 * but type list in tree_type of function_decl is struct
 * try to make them consistent
 */
TY_IDX
Get_Func_TY (gs_t type_tree, gs_t parm_list)
{
  ...
  if (idx > 1) {
	...
	gs_tree_code(type_tree) == GS_UNION_TYPE) {
      ...
      if (elt_fld.Is_Null() && gs_type_methods(type_tree) == NULL) 
  ...
  Is_True(gs_tree_code(type_tree) == GS_FUNCTION_TYPE ||
          gs_tree_code(type_tree) == GS_METHOD_TYPE, 
    ("type tree of Get_Func_TY must be FUNCTION_TYPE or METHOD_TYPE"));

  TY_IDX func_ty = Create_TY_For_Tree (type_tree, TY_IDX_ZERO);
  TYLIST_IDX parms = TY_parms(func_ty);
  gs_t arg;
  if (TY_return_to_param(func_ty)) {
    TY_IDX proto_ty = TYLIST_item(Tylist_Table[parms]);	
    if (TY_kind(proto_ty) == KIND_STRUCT) {
      Set_TYLIST_type(Tylist_Table[parms], 
          Make_Pointer_Type(proto_ty));
    }	
    parms = TYLIST_next(parms);
  }

  // Now as for METHOD_TYPE, parm_list is NULL, I don't know
  // the counter part of FUNCTION_TYPE for METHOD_TYPE
  if (parm_list == NULL) {
    return func_ty;
  }

  // as for FUNCTION_TYPE
  // TY_IDX result_ty = Get_TY(TREE_TYPE(result));
  for (arg = parm_list; Tylist_Table[parms] != TY_IDX_ZERO && arg;
       arg = gs_tree_chain(arg), parms = TYLIST_next(parms)) {
    TY_IDX arg_ty = Get_TY(gs_decl_arg_type(arg));
    TY_IDX proto_ty = TYLIST_item(Tylist_Table[parms]);

    // make prototype and arguments flag of type consistent
    TY_IDX tyflag = (TY_CONST | TY_VOLATILE | TY_RESTRICT);
    if ((arg_ty & tyflag) != (proto_ty & tyflag)) {
      proto_ty &= ~tyflag;
      proto_ty |= (arg_ty & tyflag);
      Set_TYLIST_type(Tylist_Table[parms], proto_ty);
    }
    if (TY_kind(arg_ty) == KIND_POINTER && 
        TY_kind(proto_ty) == KIND_POINTER &&
        ((TY_pointed(arg_ty) & tyflag) != 
         (TY_pointed(proto_ty) & tyflag))) {
      TY_IDX arg_pt = TY_pointed(arg_ty);
      TY_IDX proto_pt = TY_pointed(proto_ty);
      proto_pt &= ~tyflag;
      proto_pt |= (arg_pt & tyflag);
      TY_IDX newty;
      newty = Create_Ty(proto_ty, proto_pt); 
      Set_TYLIST_type(Tylist_Table[parms], newty);
      proto_ty = newty;
    }
    // add by mw
    // in func WFE_Start_Function, when create formal st,
    // if TREE_READONLY(parm_decl), add const qualifier 
    // to the type of st. for example
    // the formal type is ggVector const &
    if (gs_tree_readonly(arg)) {
      Set_TY_is_const(proto_ty);
      Set_TYLIST_type(Tylist_Table[parms], proto_ty);
    }

    // make prototype and arguments type consistent
    // Huo Wei, move the code here to set the point-to correct
    if (TY_kind(arg_ty) == KIND_POINTER && 
      TY_kind(TY_pointed(arg_ty)) == KIND_STRUCT &&
      TY_kind(proto_ty) == KIND_STRUCT) {
      Set_TYLIST_type(Tylist_Table[parms], 
	  Make_Pointer_Type(proto_ty));
      proto_ty = TYLIST_item(Tylist_Table[parms]);
    }
...
{
#ifdef W2C
                // add by mw, METHOD_TYPE should call Get_Func_TY

                gs_t type = gs_tree_type(type_tree);
                if (gs_tree_code(type) == GS_FUNCTION_TYPE ||
                    gs_tree_code(type) == GS_METHOD_TYPE) 
                  idx = Make_Pointer_Type (Get_Func_TY (gs_tree_type(type_tree), NULL));
                else 
...
#ifdef W2C
                TY_IDX void_ty_idx = ret_ty_idx; 
#endif		
		if (TY_return_in_mem(ret_ty_idx)) {
#ifdef W2C
		  void_ty_idx = Be_Type_Tbl(MTYPE_V);
#else
		  ret_ty_idx = Be_Type_Tbl (MTYPE_V);
#endif  
...
#endif
#ifdef W2C
		Set_TYLIST_type(New_TYLIST(tylist_idx),void_ty_idx);
		Set_TY_tylist (ty, tylist_idx);
		
                // add by mw, for convenience of whirl2c when used for cxx
                // if TY_return_in_mem, add a fake first parm to prototype
                // gcc syntax tree is never changed, fake parm is inserted
                // in func add_fake_parm when wn lower
                if (TY_return_in_mem(ret_ty_idx)) {
                ...
                // add by mw                                                                                                  // cmchen 08-3-7: 1058-1069
		  // modifed by Huo Wei, num_args should not reassigned
		for (arg = gs_type_arg_types(type_tree);
		     arg;
...
#ifdef W2C
        TY_IDX func_ty_idx = Get_Func_TY(gs_tree_type(decl_node), 
                                         gs_decl_arguments(decl_node));   
#else		
