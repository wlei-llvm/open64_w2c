...
// inliner/ipa, and back-end.
 ...
 #ifdef W2C
    // Next code region may change the type of pdecl, so we need to update the tylist
    TYLIST_IDX tylist_idx = TY_parms( PU_prototype(Pu_Table[ST_pu(func_st)]));
#endif
    ...
    if (TY_return_in_mem(ret_ty_idx)) {
      ...
      ++i;
#ifdef W2C
      tylist_idx = TYLIST_next(tylist_idx);
#endif
     }
...
#ifdef W2C	  
	  Set_TYLIST_type(Tylist_Table[tylist_idx], ST_type(st));	  
#endif	  
	} else
#endif
	st = Get_ST(pdecl);
...
#ifdef W2C
      ret_ty_idx = Get_TY(gs_tree_type(gs_tree_type(fndecl)));
      if (TY_return_in_mem(ret_ty_idx)) {
	      if (TY_kind(ret_ty_idx) == KIND_STRUCT) 
      			Set_TYLIST_type(New_TYLIST(tylist_idx), 
          		Make_Pointer_Type(ret_ty_idx));
    		 else
