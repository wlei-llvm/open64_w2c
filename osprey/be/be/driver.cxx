...
#ifdef W2C
extern BOOL whirl2c_under_progress;
extern BOOL whirl2c_include_stdarg; 
BOOL bDoes_Main_PU_Exist = FALSE;       
#endif
...
#ifdef W2C	
		whirl2c_under_progress=TRUE;       
#endif
	    W2C_Outfile_Translate_Pu(pu, TRUE/*emit_global_decls*/);
#ifdef W2C
		whirl2c_under_progress=FALSE;       		
#endif
	}
    }
...
/*check if current_pu contain main_pu*/
static BOOL
Preorder_Check_Main_PU_Exist(PU_Info *current_pu)
{
	if(PU_is_mainpu(PU_Info_pu(current_pu)))
		{return TRUE;}
	for(PU_Info *child=PU_Info_child(current_pu);
		child!=NULL;
		child = PU_Info_next(child))
		{
			if(Preorder_Check_Main_PU_Exist(child))
				{return TRUE;}
		}
	return FALSE;
}
...
#endif
#ifdef W2C
//added for check if pu_tree contain main_pu
bDoes_Main_PU_Exist=FALSE;
for(PU_Info *current_pu=pu_tree; current_pu!=NULL;
	current_pu=PU_Info_next(current_pu)) {
		bDoes_Main_PU_Exist = Preorder_Check_Main_PU_Exist(current_pu);
		if(bDoes_Main_PU_Exist)
			break;
}
#endif
