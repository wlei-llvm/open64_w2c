...
WHIRL2C_make_valid_c_name(const char *name)
{
   ...
   if (name != NULL)
   {
	 ...
	 while (*name != '\0')
	 {
	    ...
	    #ifdef W2C
           // add by mw, a special case in c++, struct name is suffixed by a char '*'
           // if simply remove it , name conflict may be arisen. eon bug9
           else if (*name == '*')
           {
             *new_name = '_';
	       new_name++; name++;
...
CONTEXT W2C_Bakup_Context(CONTEXT context)
{
  CONTEXT bakup = context;
  return bakup;
}

void W2C_Restore_Context(CONTEXT context, CONTEXT bakup)
{
  context = bakup;
}
