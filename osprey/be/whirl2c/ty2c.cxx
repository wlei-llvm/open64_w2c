...
#ifdef W2C
#define MTYPE_PREDEF  MTYPE_LAST
#else
...
static const SCALAR_C_NAME Scalar_C_Names[MTYPE_LAST + 1] =
   {{"void",               Name_Unknown_Type},  /* MTYPE_UNKNOWN 0 */
    ...
    #ifdef W2C
    {"signed int[4]",          "V16I4"},    /* MTYPE_V16I4 = 32 */
    #else
...
TY2C_prepend_qualifiers(TOKEN_BUFFER decl_tokens, TY_IDX ty, CONTEXT context)
{
  if (!CONTEXT_unqualified_ty2c(context))
	...
	Prepend_Token_String(decl_tokens, "volatile");
	  //wanglei  find a bug    but is it a bug???
      if (TY_is_restrict(ty)) {
#ifdef W2C
	Prepend_Token_String(decl_tokens, "__restrict");
#else
		      ...
		      CONTEXT      context)
{
   ...
   else
   {
	 ...
	 /*extern BOOL TY_should_convert(const TY_IDX ty);
        if (src_cplus &&
             TY_should_convert(TYLIST_item(Tylist_Table[params])) &&
             TY_kind(TYLIST_item(Tylist_Table[params])) == KIND_STRUCT) {
           TY2C_translate(param_tokens, 
                   Make_Pointer_Type(TYLIST_item(Tylist_Table[params])),
	           context);
            extern BOOL TY_needs_const(const TY_IDX ty);
            if (TY_needs_const(TYLIST_item(Tylist_Table[params]))) {
              Append_Token_String(param_tokens, " const");
            }*/
...
static void
TY2C_prototype_params_struct(TOKEN_BUFFER decl_tokens, 
		      TYLIST_IDX   params, 
		      CONTEXT      context)
{
//bug when   return struct   
   TOKEN_BUFFER param_tokens;
   ...
   if (src_cplus) {
     CONTEXT_reset_const(context);
       ...
       TYLIST_IDX params1 = params-1;
      param_tokens = New_Token_Buffer();
  
    TY2C_translate(param_tokens, TYLIST_item(Tylist_Table[params1]),
	           context);
	Append_And_Reclaim_Token_List(decl_tokens, &param_tokens);
	   Append_Token_Special(decl_tokens, '*'); 
...
TY2C_array(TOKEN_BUFFER decl_tokens, TY_IDX ty, CONTEXT context)
{
   if (Stab_Array_Has_Dynamic_Bounds(ty))
   {
	  ...
	  */
	 Append_Token_Special(decl_tokens, '[');
        // add by mw, empty array is refused by decc frontend, change array[] to
        // array[0] to make decc frontend happy.
#ifdef TARG_ALPHA
         TCON2C_translate(decl_tokens, 
	                                 Host_To_Targ(MTYPE_I8, 0));
#endif
	 Append_Token_Special(decl_tokens, ']');
...
static void 
TY2C_struct_obsolete(TOKEN_BUFFER decl_tokens, TY_IDX ty, CONTEXT context)
{
   ...
   TY2C_prepend_qualifiers(decl_tokens, ty, context);
} /* TY2C_struct_obsolete */


UINT32
recompute_align(TY_IDX idx)
{
...
//This function outputs it complete declaration to the w2c.h file
static void
TY2C_Output_Struct_Type(TY_IDX ty,
			     INT lines_between_decls,
			     CONTEXT context) {

  if (TY_is_translated_to_c(ty) ||
      struct_names.find(Save_Str(TY_name(ty))) != struct_names.end()
      || !TY_is_written(ty)) {
    //don't output duplicate struct definitions
    return;
  }
...
//If the given type is a user-defined struct, 
//This function outputs it complete declaration to the w2c.h file
static void TY2C_Output_Struct_Type_1(TY_IDX ty,
                                    INT lines_between_decls,
                                    CONTEXT context) {
...
static Name_Set struct_name_set;
#endif

/* 
  * add by mw, remove illegal char produce by frontend, 
  * c++ will meet the problem, eon bug3
  */
char * Remove_Illegal_Char(char *name) {
  INT i, j, length;
...
// add by mw, add union header to the same ofst fields in structure
// this appears in case of inheritance in cxx
// map from ofst to token_buffer, used to collect same ofst fields to 
// a union. eon bug18
class OFST_TOKEN_MAP
{
private:
  INT _ofst;
  INT _num;
  TOKEN_BUFFER _buf;
public:
  OFST_TOKEN_MAP(INT ofst, INT num, TOKEN_BUFFER buf) 
  	: _ofst(ofst), _num(num), _buf(buf) {}
  void Init(INT ofst, INT num, TOKEN_BUFFER buf)
  { _ofst = ofst;  _num = num;  _buf = buf; }
  INT Ofst() { return _ofst; }
  INT Num() { return _num; }
  TOKEN_BUFFER Buffer() { return _buf; }
  void Add_Num() { _num++; }
};
typedef std::list<OFST_TOKEN_MAP *> UNION_TOKEN_LIST;

// add by mw, if there is a same ofst field, return OFST_TOKEN_MAP
#define TYPE_ALLOC_N(type, count)\
   TYPE_MEM_POOL_ALLOC_N(type, Malloc_Mem_Pool, count)
   
static OFST_TOKEN_MAP * Find_Map(UNION_TOKEN_LIST *list, INT ofst) {
  UNION_TOKEN_LIST::iterator it;
  // find the map term in the list whose ofst equals to the second param
  for (it = list->begin(); it != list->end(); it++) {
    if ((*it)->Ofst() == ofst && (*it)->Num() != 0) {
      return (*it);
    }
  }
  return NULL;
...
TY2C_Translate_Structure(TY_IDX ty,CONTEXT context)
{
  ...
  /* FIXME: fe generate duplicate struct defination */
  const char *tyname = W2CF_Symtab_Nameof_Ty(ty);
  if (struct_name_set.Is_Member(tyname)){
    if(LNO_IPA_Enabled){
      //modified by wy
      if(Ty_Table[ty].u1.tylist != 0) {
        UINT ty_idx_suffix =  Ty_Table[ty].u1.tylist;
        UINT char_ty_idx_suffix_long = 0;
        char  *  ty_idx_rename;
	 if(ty_idx_suffix != 0)
          while(ty_idx_suffix/10 != 0 || ty_idx_suffix %10 != 0){
             ty_idx_suffix =ty_idx_suffix/10;
             char_ty_idx_suffix_long++;
          }
        else
	      char_ty_idx_suffix_long = 1;
        ty_idx_rename = Get_Name_Buf_Slot(strlen(tyname)+char_ty_idx_suffix_long+2) ;
        sprintf(ty_idx_rename, "%s_%d", tyname, Ty_Table[ty].u1.tylist);
        struct_name_set.Insert(ty_idx_rename);
      }
    }else{
      return ;
    }
    ...
    struct_name_set.Insert(tyname);
      }

#if 0
  if (strcmp(TY_name(ty), "gl_texture_object") == 0) {
  ...
  */
  ...
  if ((!src_cplus || TY_is_union(ty)) && flist != 0) {
...
#ifdef W2C    //wanglei  debug
if(FLD_bsize(fld) ==0 &&FLD_is_bit_field(fld))
{ 
	fld_tokens = New_Token_Buffer();
	Append_Token_String(fld_tokens,"int :  0;");
	...
	continue;
         ...
         /* Declare this field */
         fld_tokens = New_Token_Buffer();
         const char *fld_name = (const char *) FLD_name(fld_iter);
         // add by mw, remove illegal char in an identifier, eon bug3
         if (src_cplus) {
           fld_name = WHIRL2C_make_valid_c_name(fld_name);
         } 
         Append_Token_String(fld_tokens, fld_name); 
#ifdef W2C
		 	if(FLD_bsize(fld) !=0 &&FLD_is_bit_field(fld)){
				if(TY_mtype(FLD_type(fld))<=5)
				{Prepend_Token_String(fld_tokens, "_INT32"); }
				else
				{Prepend_Token_String(fld_tokens, "_UINT32"); }
		 		}
			else
#endif
         TY2C_translate(fld_tokens, FLD_type(fld),context);
         // gcc007 modified by shihui, check if this filed is bit offset.
   	 //#ifndef W2C
 	 //if(FLD_bsize(fld) !=0 && FLD_bsize(fld) <  TY_size(FLD_type(fld))*8){
	//  #else                   //wanglei debug
         if(FLD_bsize(fld) !=0 &&FLD_is_bit_field(fld)){
	//  #endif
           Append_Token_Special(fld_tokens,':');
           char tmp_len_str[20];
           sprintf(tmp_len_str, "%d", FLD_bsize(fld));
           Append_Token_String(fld_tokens,tmp_len_str);
         }
         // end
         Append_Token_Special(fld_tokens,';');
  ...
  // add by mw, I find when the object generated from .w2c.c linked with
  // that generated from .cc, the exe run in error. It is resulted from the
  // erroneous data layout of inherited class. Same offset fields was outputed
  // as if they are different fields in struct. eon bug18
  else if (src_cplus && TY_Is_Struct(ty) && flist != 0) {
    UNION_TOKEN_LIST union_buf_list;
    fld_iter = Make_fld_iter(TY_flist(ty_rt));
     do
       {
         FLD_HANDLE fld (fld_iter);
         ...
         // find the token related to the ofst, and append the current fld to the 
         // union buf
         OFST_TOKEN_MAP *map = Find_Map(&union_buf_list, FLD_ofst(fld));
         if (map != NULL) {
           TOKEN_BUFFER union_buf = map->Buffer();
           map->Add_Num();
           Increment_Indentation();
           Append_Indented_Newline(union_buf, 1);
           Decrement_Indentation();
           Append_And_Reclaim_Token_List(union_buf, &fld_tokens);
         } else {
           OFST_TOKEN_MAP *new_map = TYPE_ALLOC_N(OFST_TOKEN_MAP, 1);
           new_map->Init(FLD_ofst(fld), 1, fld_tokens);
           union_buf_list.push_back(new_map);
         }

       } while (!FLD_last_field (fld_iter++)) ;     
	 
	// output union_buf_list to struct_tokens
	UNION_TOKEN_LIST::iterator it;
	for (it = union_buf_list.begin(); it != union_buf_list.end(); it++) {
          Append_Indented_Newline(struct_tokens, 1);
          // when fld ofsts equal
          if ((*it)->Num() != 1) {
            Increment_Indentation();
            Prepend_Indented_Newline((*it)->Buffer(), 1);
            Decrement_Indentation();
            Prepend_Token_String((*it)->Buffer(), "union {");
	     Append_Indented_Newline((*it)->Buffer(), 1);
            Append_Token_String((*it)->Buffer(), "};");
			
            fld_tokens = (*it)->Buffer();			
            Append_And_Reclaim_Token_List(struct_tokens, &fld_tokens); 
...
TY2C_struct(TOKEN_BUFFER decl_tokens, TY_IDX ty, CONTEXT context)
{
  ...
  const UINT   current_indent = Current_Indentation();
  // modified by shihui bug gcc/bc-emit.c 001
  // add by mw, there are empty ty items with the same name
  // as other item, we use TY_size(ty) != 0 to ensure omit the
  // empty item, eon bug9
  if (!TY_is_translated_to_c(ty) &&
      !CONTEXT_ty_is_pointed(context) ) {
      // &&
      // TY_size(ty) != 0) {
    //Add this struct type to the global w2c.h
  ...
  // gcc007 modified by shihui bug gcc/bc-emit.c 001
  // for the sequence of structure declaration.
  // add by mw, there are empty ty items with the same name
  // as other item, we use TY_size(ty) != 0 to ensure omit the
  // empty item, eon bug9
  if (TY_is_translated_to_c(ty) ||
  ...
  if (CONTEXT_ty_is_pointed(context) && !TY_needs_no_ty_decl(ty)) {
    TOKEN_BUFFER struct_tokens = New_Token_Buffer();

    Set_TY_needs_no_ty_decl(ty);
    Set_Current_Indentation(0);
    Prepend_Token_String(struct_tokens, W2CF_Symtab_Nameof_Ty(ty));
	
    if (TY_is_union(ty) )
      Prepend_Token_String(struct_tokens, "union");
    else
      Prepend_Token_String(struct_tokens, "struct");
    // TY2C_prepend_qualifiers(struct_tokens, ty, context);
    Append_Token_Special(struct_tokens, ';');
    Append_Indented_Newline(struct_tokens, 2);
    Set_Current_Indentation(current_indent);
 ...
 */
static void 
TY2C_struct_upc(TOKEN_BUFFER decl_tokens, TY_IDX ty, CONTEXT context)
{
  const UINT   current_indent = Current_Indentation();
  ...
  if (!TY_is_translated_to_c(ty)) {
    ...
    Set_Current_Indentation(0);
    Set_TY_is_translated_to_c(ty);
    //TY2C_Output_Struct_Type_1(ty, 1, context);
    TY2C_Translate_Structure(ty,context);
    Set_Current_Indentation(current_indent);
  }
  ...
  BOOL is_anonymous = (strncmp(TY_name(ty), "T ", 2) == 0);
  if (!is_anonymous && (TY_is_translated_to_c(ty))) {
    /* the normal case */
    Prepend_Token_String(decl_tokens, W2CF_Symtab_Nameof_Ty(ty));
...
TY2C_function(TOKEN_BUFFER decl_tokens, TY_IDX ty, CONTEXT context)
{
   ...
   {
#ifdef W2C
   // debug for the struct return error 
   //  the num of  the params doesn't  match 
   // add a funtion  TY2C_prototype_params_struct 
      if(TY_kind (Func_Return_Type(ty))==KIND_STRUCT)
      	{
      	 TY2C_prototype_params_struct(decl_tokens, params, context);
      	}
	  else
	  	{
...
TY2C_translate(TOKEN_BUFFER decl_tokens, TY_IDX ty, CONTEXT context)
{
   ...
   if (special_name != NULL && *special_name != NULL)
   {
...
// bug gcc008 added by shihui
// field id's compuation is as follows
// 1. start from 1
// 2. encounter a struct, the field in struct is start from struct's field id +1

// we need fld_ofst to indicate the ofst from the beginning of the outermost
// struct, FLD_ofst(fld) is only the ofst from the beginning of the immediate 
// outer struct
TY2C_FLD_INFO
TY2C_get_field_info_by_id(TY_IDX struct_ty,      /* base type */
		    UINT32 field_id,
		    UINT32 &current_id,
		    INT32 &fld_ofst)
{
  ...
  for(fld = TY_flist(Ty_Table[struct_ty]);
      !fld.Is_Null();
      fld = FLD_next(fld))  {
    ...
    if(current_id == field_id) {
      ...
      fld_info.found_fld = fld;
      fld_info.select_tokens = New_Token_Buffer();
      Append_Token_String(fld_info.select_tokens, W2CF_Symtab_Nameof_Fld(fld));
      return fld_info;
    }
    if(TY_kind(FLD_type(fld)) == KIND_STRUCT) {
      fld_ofst += FLD_ofst(fld);
      TY2C_FLD_INFO inner_fld_info = TY2C_get_field_info_by_id(FLD_type(fld), field_id, current_id, fld_ofst);
      if(!inner_fld_info.found_fld.Is_Null()){ // got field id
        Prepend_Token_Special(inner_fld_info.select_tokens, '.');
        Prepend_Token_String(inner_fld_info.select_tokens, W2CF_Symtab_Nameof_Fld(fld));
        return inner_fld_info;
      }
    }
  }

  fld_info.found_fld = FLD_HANDLE(0);
  return fld_info;
}
...
TY2C_builtin(TY_IDX ty)
{
  ...
  TYPE_ID predef = MTYPE_PREDEF;

   const char *name = GET_SPECIAL_TYPENAME(mtype);
   const char *pseudo_name = Scalar_C_Names[mtype].pseudo_name;

   if (name != NULL&& *name != NULL)
     return TRUE;
   if (mtype <= predef) {
     if (mtype != MTYPE_UNKNOWN || TY_kind(ty) == KIND_INVALID) {
       if (pseudo_name != NULL && *pseudo_name != NULL) {
         return TRUE;
       }
     }
   }
