...
// We want to transfer some optimization info into whirl2c 
// part by this map
WN_MAP Lno_Opt_Map = WN_MAP_UNDEFINED; 
#pragma weak Lno_Opt_Map

// m16. descriminate lda/ldid/stid in WN2C_based_lvalue_st
#define LDA_KIND 0x1
#define LDID_KIND 0x2
#define STID_KIND 0x3

// add by mw
extern BOOL src_cplus;

//xlzhang:functions in C_Lib_Reserved_Func_Names should be declared in a #ifdef..#endif manner.
struct C_Lib_Reserved_Func_And_HeaderMacro
{
   char *symbol;
   char *headermacro;
};
static const  C_Lib_Reserved_Func_And_HeaderMacro C_Lib_Reserved_Func_Name[] =
{
                 /* <math.h> */
   {"cos","_MATH_H"},
   {"pow","_MATH_H"},
   ...
   {"strcspn","_STRING_H"},
   {"strspn","_STRING_H"},
   {"strncasecmp","_STRING_H"},
   {"malloc", "__WHIRL2C_H__"},
   {"calloc", "__WHIRL2C_H__"},
   {"realloc", "__WHIRL2C_H__"},
   {"free", "__WHIRL2C_H__"},
   #if 0
   {"__strtol_internal", "_STDLIB_H"},
   {"__strtoull_internal", "_STDLIB_H"},
   {"strtol", "_STDLIB_H"},
   {"select", "_STDLIB_H"},
   {"atol", "_STDLIB_H"},
   {"atoi", "_STDLIB_H"},
...
#define C_Lib_Reserved_Func_And_HeaderMacro_Length (sizeof(C_Lib_Reserved_Func_Name)/sizeof(C_Lib_Reserved_Func_And_HeaderMacro))


static BOOL is_CLIB_Reserved_Func(const ST *st)
{
  for (int idx = 0; idx < C_Lib_Reserved_Func_And_HeaderMacro_Length; idx++)
    if (strcmp(C_Lib_Reserved_Func_Name[idx].symbol, ST_name(st)) == 0) return TRUE;
  return FALSE;
}
static char* ST_name_to_header_file_macro(const ST *st)
{
  for (int idx = 0; idx < C_Lib_Reserved_Func_And_HeaderMacro_Length; idx++)
    if (strcmp(C_Lib_Reserved_Func_Name[idx].symbol, ST_name(st)) == 0) return C_Lib_Reserved_Func_Name[idx].headermacro;
  return "ERROR";
}
...
#ifdef W2C
static BOOL       Need_To_Cast = FALSE;
BOOL                RETURN_SAVE_NOTSKIP = FALSE;
#endif
...
#if 1 /* FIXME: delele class Name_Set */
...
class Name_Set {
  std::set<const char*, ltstr> name_set;
public:
  Name_Set (void) {}

  Name_Set (int count, const char *vals[])
  {
    int i;
...
static STATUS WN2C_reduce_add(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context);
static STATUS WN2C_intconst(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context);
static STATUS WN2C_replicate(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context);
...
static const OPR2HANDLER WN2C_Opr_Handler_Map[] =
{
   ...
   {OPR_REDUCE_ADD,&WN2C_reduce_add},
   {OPR_REPLICATE,&WN2C_replicate},
   #endif
#ifdef KEY
   {OPR_LDMA, &WN2C_lda},
...
#ifdef W2C
//{OPC_F4F4FLOOR, "_F4F4FLOOR"},
{OPC_F4F4FLOOR, "_I4F4FLOOR"},
{OPC_FQFQFLOOR, "_I8FQFLOOR"},
#endif
  {OPC_I4BNOT, "~"},
  {OPC_U8BNOT, "~"},
  {OPC_I8BNOT, "~"},
...
// TODO WHIRL 0.30: get rid of OPC_I4T1 variants
  ...
  {OPC_V16U4SUB, "-"},
  {OPC_V16U4NEG, "-"},
  {OPC_V16I4ABS, "_V16I4ABS"},
  {OPC_V16U4ABS, "_V16U4ABS"},
  
  {OPC_V16I4SHL, "_V16I4SHL"},
  {OPC_V16I4LSHR, "_V16I4LSHR"},
  {OPC_V16I4ASHR, "_V16I4ASHR"},
  {OPC_V16U4SHL, "_V16U4SHL"},
  {OPC_V16U4LSHR, "_V16U4LSHR"},
  {OPC_V16U4ASHR, "_V16U4ASHR"},
  
  {OPC_V16U4BAND, "&"},  
  {OPC_V16U4BIOR, "|"},
  {OPC_V16U4BXOR, "^"},

  {OPC_V16I16SHL, "_V16I16SHL"},
  {OPC_V16I16LSHR, "_V16I16LSHR"},
  {OPC_V16I16ASHR, "_V16I16ASHR"},
  {OPC_V16U16SHL, "<<"},
  {OPC_V16U16LSHR, "_V16U16LSHR"},
  {OPC_V16U16ASHR, "_V16U16ASHR"},
				  ...
				  CONTEXT  context)
{
   ...
   if (!WN2C_arithmetic_compatible_types(result_ty, opnd_ty))
   {
      // add by mw, because there is a cast follow translate, so remove
      // the flag context_need_lvalue. the same problem to C, so don't 
      // add src_cplus. eon bug12
      // CONTEXT_reset_needs_lvalue(context);
      CONTEXT_set_top_level_expr(context);
	#ifdef W2C
	    if (TY_Is_Structured(WN_ty(wn)))
	{
        if (WN_field_id(wn) ==0) 
			{
		Append_Token_String(opnd_tokens, "*(");
		//	WN2C_prepend_cast(opnd_tokens, WN_Tree_Type(wn), FALSE);
		Append_Token_String(opnd_tokens,  Scalar_C_Names[WN_desc(wn)].pseudo_name);		
		//	Append_Token_String(opnd_tokens, "*(_UINT8*)&");
		Append_Token_String(opnd_tokens, "*)&");
	              }
      }
	#endif
      status = WN2C_translate(opnd_tokens, wn, context);
      WHIRL2C_parenthesize(opnd_tokens);
      // OPR_LDA: delay to add '&' in WN2C_based_lvalue_st to convert
      // (&A)->a to A.a. The code here is a compensation.
      // May be missed by combination, added by Huo Wei
     if (STATUS_needs_addr(status) && (WN_operator(wn) == OPR_LDA)) {
    		Prepend_Token_Special(opnd_tokens, '&');
		WHIRL2C_parenthesize(opnd_tokens);
    		STATUS_reset_needs_addr(status);
     }
     WN2C_prepend_cast(opnd_tokens, result_ty, FALSE);
   }
   else {
		 ...
		 CONTEXT      context)
{
   ...
   if (intconst != NULL)
   {
	 ...
	 WN_const_val(intconst) = 
       ...
       */
       #else
        if (OPCODE_operator(opcode) == OPR_SUB) {
      WN2C_append_cast(tokens, expr_ty, FALSE/*ptr_to_type*/);

      if (TY_Is_Pointer(wn0_ty))
	 wn0_ty = Stab_Mtype_To_Ty(OPCODE_rtype(opcode)); 
      else if (TY_Is_Pointer(wn1_ty))
	 wn1_ty = Stab_Mtype_To_Ty(OPCODE_rtype(opcode));
   ...
   else
   {
       ...
       */
#ifdef W2C
 Is_True(0, ("look at when will take the path"));
#endif
   ...
   return EMPTY_STATUS;
   #else
   // m5. restore needs_lvalue in CONTEXT, needs_lvalue may be set in WN2C_array
   //     tells no need to add * before the address add
   STATUS status = EMPTY_STATUS;
   #if 0
   if (needs_lvalue) {
     STATUS_set_lvalue(status);
     CONTEXT_set_needs_lvalue(context);  /* Need actual value of subexprs */
   }
   #endif
   STATUS_set_needs_ref(status);
   STATUS_reset_no_direct_cast(status);
   return status;
   #endif
 ...
 STATUS       status = EMPTY_STATUS;
   ...
   else if (binary_op && TY_Is_Pointer(result_ty))
   {
      ...
      return WN2C_address_add(tokens, 
			      ...
			      context);
      #else
       status = WN2C_address_add(tokens, 
			      opcode,  
			      result_ty,
			      wn0,
			      wn1,
   ...
   if (binary_op)
   {
     ...
     #if 0
    if((opcode==OPC_V16I4ADD) &&(WN_operator(wn0) == OPR_CONST))
   	{
   	Prepend_Token_String(opnd_tokens,"4 * ");
   	}
	 #endif
      Append_And_Reclaim_Token_List(tokens, &opnd_tokens);
   }
   ...
   opnd_tokens = WN2C_Translate_Arithmetic_Operand(wn1, wn1_ty, context);
     #if 0
    if((opcode==OPC_V16I4ADD) &&(WN_operator(wn1) == OPR_CONST))
   	{
   	Prepend_Token_String(opnd_tokens,"4 * ");
   	}
	 #endif
		    ...
		    STAB_OFFSET offset)
{
   ...
   else
   {
	 ...
	 return load_ty;
#else
	  // m23 sometime the offset is wrong, then not to restore high type info 
      //     but use mtype instead
      else if (Stab_Mtype_To_Ty(load_mtype) != (TY_IDX) 0 &&
               TY_size(Stab_Mtype_To_Ty(load_mtype)) == TY_size(load_ty))
      {
	 return load_ty;

      } else if (load_mtype == MTYPE_M) {
         return load_ty;
      } else {
...
#ifdef W2C
static STATUS
WN2C_based_lvalue_st(INT wnkind,               /* is lda or ldid or stid */
                  TOKEN_BUFFER expr_tokens,    /* lvalue or addr expr */
		  TY_IDX       wn_st_type,        /* type of base of st */
                  ...
                  UINT         field_id = 0)
{
   ...
   BOOL          direct_cast = FALSE;
   TY_IDX        base_obj_ty = wn_st_type;
   TY2C_FLD_INFO field_info;
   field_info.select_tokens = NULL;
   field_info.found_fld = FLD_HANDLE();

   /* PRECONDITION: */
   Is_True(base_obj_ty != (TY_IDX) 0 &&
	   object_ty != (TY_IDX) 0,
	   ("Expected non-null types in WN2C_based_lvalue_st()"));
   ...
   // m8. change WN2C_compatible_lvalues to WN2C_unparser_compatible_types
   //     reduce unnecessary type conversion 
   incompat_obj_tys = !WN2C_unparser_compatible_types(object_ty, base_obj_ty);
   if (wnkind == STID_KIND && TY_Is_Array(base_obj_ty) && TY_Is_Pointer(object_ty)) {
     // we cannot give such assignment <array> left = <pointer> right
     // be careful about the following WN2C_unparser_compatible_types
     // maybe the same problem
     incompat_obj_tys = TRUE;
   }

   if (TY_Is_Pointer(object_ty) &&
         MTYPE_is_pointer(TY_mtype(base_obj_ty))) {
     direct_cast = TRUE;
   }
   ...
   #endif
   // m22 what use? I nullify it here and see what will happen 
   #if 0
 ...
 /////wanglei job break
   ...
   if ((addr_offset != 0 || incompat_obj_tys) && TY_Is_Structured(base_obj_ty))
   {
     if (wnkind == LDA_KIND) {
	  ...
	  TY_IDX fld_type = FLD_type(field_info.found_fld);
	  if (WN2C_unparser_compatible_types(fld_type, object_ty)) {
           WHIRL2C_parenthesize(expr_tokens);
           Append_Token_Special(expr_tokens, '.');
           Append_And_Reclaim_Token_List(expr_tokens, &field_info.select_tokens);
           if (TY_kind(fld_type) != KIND_ARRAY) {
             STATUS_set_needs_addr(status);
           }
           return status;
	  }
       }
     // wnkind == LDID/STID
       ...
       if (TY_Is_Structured(base_obj_ty)) {
         if (field_id > 0) {
           UINT cur_fld_id = 0;
           INT32 fld_ofst = 0;
           TY2C_FLD_INFO fld_info = TY2C_get_field_info_by_id(base_obj_ty, 
                                                              field_id, 
                                                              cur_fld_id,
                                                              fld_ofst);
           FLD_HANDLE fld = fld_info.found_fld;
           TY_IDX fld_type = FLD_type(fld);
           char * fld_name = FLD_name(fld);
           if ((addr_offset == fld_ofst) &&
               WN2C_unparser_compatible_types(fld_type, object_ty)) {
             WHIRL2C_parenthesize(expr_tokens);
             Append_Token_Special(expr_tokens, '.');
             Append_And_Reclaim_Token_List(expr_tokens, &fld_info.select_tokens);
             return status;
           }
   ...
   if (addr_offset != (STAB_OFFSET) 0 || incompat_obj_tys)
   {
     // if (expr_is_lvalue) {
       #if 0
       if (TY_Is_Array(TY_pointed(addr_ty)) &&
	   WN2C_arithmetic_compatible_types
           (Get_Inner_Array_Type(TY_pointed(addr_ty)), object_ty)) {
	 /* For multidimensional arrays like int aa[N][M], 
          * we transform an array access aa[i][j] to 
	  ...
	  */
       ...
       #if 0
       if (addr_offset == 0 &&
	   	  TY_size(base_obj_ty) == TY_size(object_ty) &&
	   	  MTYPE_is_integral(TY_mtype(base_obj_ty)) &&
                MTYPE_is_integral(TY_mtype(object_ty)) &&
                TY_Is_Scalar(base_obj_ty) &&
                TY_Is_Scalar(object_ty)) {
	 //Do not use indirection when converting between integral types
	  if (wnkind == LDA_KIND) {
	    // maybe unlikely happen, the object_ty should be a pointer type
           Prepend_Token_Special(expr_tokens, '&');
	  }
	...
	#endif
	{
         // m14. add is_lda to control WN2C_based_lvalue_st
         if (wnkind == LDA_KIND && TY_kind(base_obj_ty) != KIND_ARRAY) {
           Prepend_Token_Special(expr_tokens, '&');
           STATUS_reset_needs_addr(status);
         }
     ...
     // m9. comment out expr_is_lvalue = FALSE
     if (addr_offset != 0)
     {
	...
	*/
       ...
       if (wnkind == LDA_KIND) {
         ...
         WN2C_prepend_cast(expr_tokens, object_ty, TRUE);
       }
     }
     else if (incompat_obj_tys)
     {
       if (wnkind == LDA_KIND) {
         WN2C_prepend_cast(expr_tokens, object_ty, FALSE);
       } else if (wnkind == STID_KIND) {
         WN2C_prepend_cast(expr_tokens, object_ty, TRUE);

       } else if (wnkind == LDID_KIND) {
         if (direct_cast) {
           WN2C_prepend_cast(expr_tokens, object_ty, FALSE);
         } else {
           WN2C_prepend_cast(expr_tokens, object_ty, TRUE);
     ...
     // See bug383
     WN2C_prepend_cast(expr_tokens, object_ty, FALSE);
   } else if (wnkind == LDA_KIND && TY_kind(base_obj_ty) != KIND_ARRAY) {
     STATUS_set_needs_addr(status);
   }
 ...
 ret_point:
   ...
   return status;
} /* WN2C_based_lvalue_st */


static STATUS
WN2C_based_lvalue_wn(TOKEN_BUFFER expr_tokens,    /* lvalue or addr expr */
		  STATUS       status,         /* an lvalue expr? */
		  TY_IDX       expr_ty,        /* type of expr */
                  TY_IDX       addr_ty,        /* expected addr type */
		  TY_IDX       object_ty,      /* type of object */
		  STAB_OFFSET  addr_offset,    /* offset of object from base */
                  CONTEXT      context,
                  BOOL         is_bit_field,
                  UINT         field_id = 0)
{
    ...
    */
   BOOL          incompat_obj_tys;  /* object_ty vs TY_pointed(addr_ty) */
   BOOL          incompat_addr_tys;  /* object_ty vs TY_pointed(addr_ty) */
   BOOL          expr_is_array = FALSE;
   BOOL          not_get_addr = FALSE;
   BOOL          direct_cast = FALSE;
   TY_IDX        tree_obj_ty;
   TY2C_FLD_INFO field_info;
   field_info.select_tokens = NULL;
   ...
   /* PRECONDITION: */
   Is_True((addr_ty != (TY_IDX) 0 && TY_Is_Pointer(addr_ty) &&
            object_ty != (TY_IDX) 0),
	   ("Expected non-null types in WN2C_based_lvalue_wn()"));

   Is_True((expr_ty != (TY_IDX) 0 && 
            (TY_Is_Pointer(expr_ty) || 
             /*(MTYPE_is_integral(TY_mtype(expr_ty)) && 
             (MTYPE_byte_size(TY_mtype(expr_ty)) == MTYPE_byte_size(Pointer_type))))), */
             MTYPE_is_integral(TY_mtype(expr_ty)))),
               ("Expected non-null types in WN2C_based_lvalue_wn()"));

  // iload/istore have two cases: iload (addr expr) or iload (array ...), they require differnt
  // translation ways because addr expr is the indirect load addr, but A[i] is the load object
  // The info is passed by STATUS_needs_ref and we use expr_is_array to discriminate 
  // these two cases.
  if (!STATUS_needs_ref(status)) {
    expr_is_array = TRUE;
  }
  // &(array type B[])A is the same with (array type B[])A, which gets A + i * sizeof(B)
  if (TY_Is_Pointer(expr_ty)) {
    not_get_addr = TY_Is_Array(TY_pointed(expr_ty));
  }
   /* Determine type compatibility between the expression type (expr_ty),
    * its expected type (addr_ty), and the type of object being accessed
    * object_ty.  While the addr_ty must be a pointer type, the expr_ty
    * may be any value that may be casted to a pointer type.
    */
   // m19. change addr_ty to expr_ty, maybe incompat_addr_tys can be nullified
   //      m19 is removed, change to m20
   incompat_addr_tys = !WN2C_unparser_compatible_types(addr_ty, expr_ty);

   /* Correct the address of the base object, if necessary.  Note that
    * we permit any kind of expr_ty, while the addr_ty must always be
    * a pointer type.
    */
   // m24. remove incompat_addr_tys because normalize make expr_ty the proper
   //      addr ty
   if (incompat_addr_tys)
   {
	 /* Cast the address value to the appropriate pointer type */
      if (!STATUS_needs_ref(status) && !not_get_addr) {
        Prepend_Token_Special(expr_tokens, '&');
      } 
      WN2C_prepend_cast(expr_tokens, addr_ty, FALSE/*ptr_to_type*/);
      WHIRL2C_parenthesize(expr_tokens);
   ...
   // m8. change WN2C_compatible_lvalues to WN2C_unparser_compatible_types
   //     reduce unnecessary type conversion
   // m25. add !WN2C_unparser_compatible_types(object_ty, tree_obj_ty) to the condition
   //      of incompat_obj_tys
   // incompat_obj_tys = !WN2C_unparser_compatible_types(object_ty, base_obj_ty); 
   tree_obj_ty =  TY_pointed(expr_ty);
   incompat_obj_tys = !WN2C_unparser_compatible_types(object_ty, tree_obj_ty);

   // TY_Is_Array(tree_obj_ty) means ((_INT8 (*)[8])A)[i] ==> A + i*8, not the 
   // value of the 8 bytes mem region but its head addr, so STATUS_set_needs_ref
   // and we don't use the format *((_INT8 (*)[8])A)[i] to avoid warning, so we 
   // need to set incompat_obj_tys = FALSE
   // if (!STATUS_needs_ref(status) && TY_Is_Array(tree_obj_ty)) {
   if (!STATUS_needs_ref(status) && not_get_addr) {
     incompat_obj_tys = TRUE;
     STATUS_set_needs_ref(status);
   }

   // needs_ref means the input expr is not array form, so it is the addr intended to load
   // !STATUS_needs_ref(status) doesn't equals to expr_is_array here because of 
   // STATUS_set_needs_ref(status) in the incompat_addr_tys == TRUE case
   if (STATUS_needs_ref(status) && !expr_is_array) {
     // means address expr 
     direct_cast = TRUE;
   } else if (TY_Is_Pointer(object_ty) &&
         MTYPE_is_pointer(TY_mtype(tree_obj_ty))) {
     // if object_ty is pointer and expr_is_array, we don't add *(object *)&A[i] but (object)A[i]
     direct_cast = TRUE;
   }
   #if 0
   else 

   if (WN2C_array_lvalue_as_ptr(addr_ty,            /* ptr to array */
				     Stab_Pointer_To(object_ty))) /* ptr */
   {
      /* An array lvalue is to be interpreted in C as a pointer to 
       * the array element type, so no cast is necessary
    ...
    * always try to use field selection instead of pointer arithmetic
    */
   // m20. only according to the addr + addr_offset method
           
   #if 0
   if ((addr_offset != 0 || incompat_obj_tys) && TY_Is_Structured(tree_obj_ty))
   {
      field_info =
	 TY2C_get_field_info(tree_obj_ty, 
			     object_ty, 
			     TY_mtype(object_ty), 
			     addr_offset);
   }
   ...
   if (!is_bit_field && 
       (addr_offset != 0 || incompat_obj_tys) && 
       TY_Is_Structured(tree_obj_ty)) {
     if (TY_Is_Structured(tree_obj_ty)) {
       if (field_id > 0) {
         UINT cur_fld_id = 0;
         INT32 fld_ofst = 0;
         TY2C_FLD_INFO fld_info = TY2C_get_field_info_by_id(tree_obj_ty, 
                                                            field_id, 
                                                            cur_fld_id,
                                                            fld_ofst);
         FLD_HANDLE fld = fld_info.found_fld;
   ...
   if (addr_offset != (STAB_OFFSET) 0 || incompat_obj_tys)
   {
	  ...
	  */
       ...
       // it is of no use
                ...
                TY_kind(base_obj_ty) == KIND_SCALAR) {
	 //Do not use indirection when converting between integral types
	 WN2C_prepend_cast(expr_tokens, object_ty, FALSE);
	 goto ret_point;
       }
       #endif
     // if a[i] tree_obj_ty incompatible with object_ty, we should add needs_ref

     if (!direct_cast && !not_get_addr) {
       Prepend_Token_Special(expr_tokens, '&');
     }
	...
	*/
     ...
     {
       // because the typecast is for addr expr or array element addr,
       // so (object_ty *) should be added
       if (direct_cast && expr_is_array) {
         WN2C_prepend_cast(expr_tokens, object_ty, FALSE);
       } else {
         WN2C_prepend_cast(expr_tokens, object_ty, TRUE/*ptr_to_type*/);         
       }
   ...
   else if (TY_is_logical(tree_obj_ty) && !TY_is_logical(object_ty)) {
     // treat long and int as differnt types, even if they have the same size for ia32
     // See bug383
     WN2C_prepend_cast(expr_tokens, object_ty, FALSE);
   }
   ...
   if (is_bit_field) {
     // a special process to bit field, correct bit field process depends on
     // field info, not load/store offset
     UINT32 current_fld_id = 0;
     INT32 fld_ofst = 0;
     TY2C_FLD_INFO field_info;
     field_info.select_tokens = NULL;
     field_info.found_fld = FLD_HANDLE();
     ...
     WHIRL2C_parenthesize(expr_tokens);
     field_info = TY2C_get_field_info_by_id(object_ty, 
                      field_id, current_fld_id, fld_ofst);
     Is_True(!field_info.found_fld.Is_Null(), 
             ("bitfield requires field_info is null"));
     if (!STATUS_needs_ref(status)) {
       Append_Token_Special(expr_tokens, '.');
     } else {
       Append_Token_String(expr_tokens, "->");
     ...
     /* Is_True(addr_offset == 0, 
              ("addr_offset != 0 when ILOAD/ISTORE is a bitfield OP")); */
 ...
 ret_point:
   ...
   return status;
} /* WN2C_based_lvalue_wn */


static STATUS
WN2C_lvalue_st(INT wnkind,
               TOKEN_BUFFER tokens,
	       const ST    *st,          /* Base symbol for lvalue */
	       TY_IDX       wn_st_type,     /* Type of base object */
	       TY_IDX       object_ty,   /* Type of object */
	       STAB_OFFSET  addr_offset, /* Offset of object from base */
	       CONTEXT      context,
               UINT         field_id = 0)
{
   ...
   {
   #ifdef W2C  //wanglei debug  for  translate wchat_t type 
   TCON tcon =STC_val(st);
   //	TY_IDX ty;
   if(TCON_ty(tcon)==MTYPE_STR&&TY_Is_Array(wn_st_type))
   	{
   	if(TY_size(TY_AR_etype(wn_st_type)) == 4)
   	{ 
   	const char  *strbase;
   	char        *str;
   	const char *str_base;
	 ...
	 str_base = str = (char *)alloca(2*strlen + 4); /* "'", orig_str, "'", and "\0" */
   	*(str++) = 'L';
   	...
   	for (stridx = 0; stridx < strlen; stridx+=4) {
		      if ((strbase[stridx] == '\0') )  // it's not the last char
        break;
      *(str++) =  strbase[stridx];
      // simd whirl2c
   ...
   *(str++) = '\0';
   ...
   else
   {
	 ...
	 addr_ty = Stab_Pointer_To(ST_type(st));
      ...
      if (strcmp(ST_name(st), "p0") == 0) {
        printf("WN2C_lvalue_st\n");
...
#endif
      //Append_Token_String(tokens, "/*wanglei*/");
      ST2C_use_translate(tokens, st, context);

      /* Get the object as an lvalue or as an address expression.
       */
      status = WN2C_based_lvalue_st(wnkind,
                                 tokens,      /* base variable name */
				 // TRUE,        /* tokens represent an lvalue */
				 wn_st_type,     /* expected type of base addr */
               ...
               UINT         field_id)
{
   ...
   // m10. change CONTEXT_set_needs_lvalue to CONTEXT_reset_needs_lvalue
   //      I think WN2C_lvalue_wn is called by ILOAD/ISTORE
   //      which needs address because they are indirect reference
   // CONTEXT_reset_needs_lvalue(context);
   ...
   {
      if (Lno_Opt_Map != WN_MAP_UNDEFINED && WN_MAP32_Get(Lno_Opt_Map, wn) == 1)
        CONTEXT_set_array_fold(context);
   ...
   status = WN2C_based_lvalue_wn(tokens,       /* base expression */
                              status,
...
// add by mw, eon bug15
#define MAX_ASM_PREG_NUM 10
static INT asm_preg_num = 0;
static INT neg_to_pos[MAX_ASM_PREG_NUM];

// add by mw, change PREG_IDX preg_idx to INT32 preg_idx
// for I want to convert negative preg_idx in WN2C_Append_Preg
// func
static void 
WN2C_Append_Preg(TOKEN_BUFFER tokens, 
		 const ST    *st,       /* preg */
		 INT32        preg_idx, /* preg index */
		 ...
		 CONTEXT      context)
{
   ...
   // add by mw, preg_idx < -1 is used for inline asm for code generation
   // when translated to c, convert it to a positive one, save the map relation
   // from negative preg_idx to positive one into array neg_to_pos. eon bug15
   if (preg_idx < 0) {
     INT idx = -preg_idx;
     Is_True(idx < MAX_ASM_PREG_NUM, 
	 	("asm preg idx is out of MAX_ASM_PREG_NUM"));
     char name[20];
     if (neg_to_pos[idx] == 0) {
       sprintf(name, "inline_asm_preg%d", asm_preg_num);
       preg_idx = Create_Preg(TY_mtype(ST_type(st)), name);
       asm_preg_num++;
       neg_to_pos[idx] = preg_idx;
     } else {
       preg_idx = neg_to_pos[idx];
     }
   }
    ...
    */
   if (!WN2C_unparser_compatible_types(preg_ty, ty))
		       ...
		       CONTEXT       context)
{
   ...
   if (!WN2C_assignment_compatible_types(assign_ty, rhs_ty))
   {
	 ...
	 WHIRL2C_parenthesize(rhs_buffer);
       ...
       //added by axm to deal with the case when a expr with pointer type is assigned to a float var
       if(TY_kind(WN_Tree_Type(rhs))==KIND_POINTER &&
	   	(TY_mtype(assign_ty) == MTYPE_F4 ||
	   	 TY_mtype(assign_ty) == MTYPE_F8 ||
	   	 TY_mtype(assign_ty) == MTYPE_FQ)){
         Prepend_Token_String(rhs_buffer, "(unsigned long)");   
   ...
   status = WN2C_translate(rhs_buffer, rhs, context);

  // OPR_ARRAY: delay to add '&' in WN2C_array to convert
  // *(&a[i]) to a[i]. The code here is a compensation to the case:
  //       ...
  //    OPR_ARRAY
  // STID
  if (!STATUS_needs_ref(status) && (WN_operator(rhs) == OPR_ARRAY)) {
    Prepend_Token_Special(rhs_buffer, '&');    
  }
  
  // OPR_LDA: delay to add '&' in WN2C_based_lvalue_st to convert
  // (&A)->a to A.a. The code here is a compensation. The & must reside
  // before type cast is added, &(_UINT64)A is illegal   
  if (STATUS_needs_addr(status) && (WN_operator(rhs) == OPR_LDA)) {
    Prepend_Token_Special(rhs_buffer, '&');
    STATUS_reset_needs_addr(status);
  }

   if (!WN2C_unparser_compatible_types(assign_ty, rhs_ty)) {
     /* Huo Wei
   	  * For non scalar type, the direct CVT is not allowed
   	  * So for TYPE t of Var s, we use *(t *)&(s)  instead of (t)s.
	  */
   	if( TY_kind(assign_ty) == KIND_SCALAR || TY_kind(assign_ty) == KIND_POINTER) {
     		WHIRL2C_parenthesize(rhs_buffer);
     		WN2C_prepend_cast(rhs_buffer,
              	           assign_ty,
                     	    FALSE/*pointer_to_type*/);
   ...
   /*
    * add by mw, translate MISTORE array to memmove, eon bug14
    */
   if (src_cplus &&
       WN_opcode(W2CF_Get_Parent(rhs)) == OPC_MISTORE) {
     WN *parent = (WN *)W2CF_Get_Parent(rhs);
     Is_True(TY_kind(TY_pointed(WN_ty(parent))) == KIND_STRUCT,
             ("WN_ty of MISTORE should be a struct"));
 
     if (WN_field_id(parent) != 0) {
       UINT cur_field_id = 0;
       FLD_HANDLE fld = FLD_get_to_field(TY_pointed(WN_ty(parent)), 
                                         WN_field_id(parent), 
                                         cur_field_id);
       INT size = TY_size(FLD_type(fld));
       ...
       TCON2C_translate(tokens, 
                        Host_To_Targ(WN_rtype(WN_kid1(parent)), 
                        WN_store_offset(parent)));
       Append_Token_String(tokens, ", ");
       TCON2C_translate(tokens, Host_To_Targ(MTYPE_I4, size));
       Append_Token_Special(tokens, ')');
       return; 
     }
   }
     ...
     if (TY_Is_Array(assign_ty) &&
          ...
          TY_Is_Array(WN_ty(rhs))) {
        TY_IDX array_ty =assign_ty;
        INT32 size = TY_size (assign_ty);
        char bytes[64];
		
      //   temp_tokens = New_Token_Buffer();
	// TY2C_translate(temp_tokens, array_ty, context);
	 
        sprintf(bytes, "%d", size);
        Append_Token_String (tokens, "memcpy");
        Append_Token_Special (tokens, '(');
	// Append_Token_Special (tokens, '(');
	//  Append_And_Copy_Token_List (tokens, temp_tokens);
	//  Append_Token_Special (tokens, '*');
	//Append_Token_Special (tokens, ')');
	 Append_And_Reclaim_Token_List(tokens, lhs_tokens);
      //Append_Token_String (tokens, W2CF_Symtab_Nameof_St(WN_st(wn)));
        Append_Token_Special (tokens, ',');
      //  Append_Token_String (tokens,   W2CF_Symtab_Nameof_St(WN_st(WN_kid0(wn))));
        Append_And_Reclaim_Token_List(tokens, &rhs_buffer);
        Append_Token_Special (tokens, ',');
        Append_Token_String (tokens, bytes);
        Append_Token_Special (tokens, ')');
     	}
	 else
     		{
...
WN2C_Append_Symtab_Types(TOKEN_BUFFER tokens, UINT lines_between_decls)
{
   ...
   for (ty = 1; ty < TY_Table_Size(); ty++)
   {
     ty_idx = ty << 8;
#if 0
     if (strcmp(TY_name(ty_idx), "gl_texture_object") == 0) {
...
#if 1 /* FIXME: set this in Frontend */
...
#ifdef W2C
/*
 * add by mw, find all the sts in a INITV tree
 * the root of the tree is indicated by INITO_val(inito)
 */
using std::map;
using std::set;
using std::list;
#define STL_TYPE_ALLOC(type)     \
      TYPE_MEM_POOL_ALLOC_N(type, Malloc_Mem_Pool, 0)
typedef set<ST_IDX> ST_SET;
typedef list<ST_IDX> ST_LIST;
typedef map<ST_IDX, ST_SET *> ST_MAP;

void 
Print_Set(FILE *file, ST_SET *set) {
  if (file == NULL)
    file = fopen("file", "a+t");
...
void 
Print_List(FILE *file, ST_LIST *list) {
  if (file == NULL)
    file = fopen("file", "a+t");
...
void 
Print_Seq(FILE *file, ST_MAP *sequence) {
  // print sequence here for debug
  if (file == NULL)
    file = fopen("file", "a+t");

  ST_MAP::iterator seq_it;
  for (seq_it = sequence->begin(); 
         seq_it != sequence->end(); seq_it++) {
    fprintf(file, "st_idx%d -->\n", seq_it->first);

    ST_SET *set = seq_it->second;
    ST_SET::iterator set_it;
...
static void 
Find_St_In_Initv(INITV_IDX initv, ST_SET *setptr, ST_IDX st_idx)
{
  switch (INITV_kind(Initv_Table[initv]))
  {
  case INITVKIND_SYMOFF:
  {
    // here we assume INITV_ofst == 0, use assertion 
    // to catch which are beyond our expectation
    ST_IDX initv_st = INITV_st(Initv_Table[initv]);
    // if (INITV_ofst(Initv_Table[initv]) == 0) {
      // initv_st should be defined before this use
    if ((ST_level(&St_Table[initv_st]) == CURRENT_SYMTAB) && 
        (initv_st != st_idx)) {
      setptr->insert(initv_st);
    }
    // } else {
      // may be we could simply dismiss the ofst and
      // keep going the branch above
      // Is_True(0, ("INITV_ofst != 0, st may not be which we want"));
  ...
  {
    INT inv_repeat = 0;
    INITV_IDX inv = INITV_blk(initv);
    while (inv != 0) {
      Find_St_In_Initv(inv, setptr, st_idx);
      inv_repeat = INITV_repeat(inv);
...
/*
 * add by mw, remove st in set in sequence, 
 * if the set is empty, the st_idx mapped to 
 * the set can be moved to the list in 
 * WN2C_Build_Queue
 */
void static
Remove_St_In_Sequence(ST_MAP *sequence, ST_IDX st_idx)
{
  ST_MAP::iterator seq_it;
  ST_SET::iterator set_it;
  ST_SET *set;
  for (seq_it = sequence->begin(); 
    ...
    set_it = set->find(st_idx);
    // if st_idx in set, remove it
    if (set_it != set->end()) {
      set->erase(set_it);
...
/*
 * add by mw, use the sequence to build a list
 * list is a list of st_idx, st processing should
 * be in the order of the list
 */
 
 static void
WN2C_Build_Queue(ST_MAP *sequence, ST_LIST *list)
{
  ST *st;
  ST_IDX st_idx;
  ST_MAP::iterator seq_it;
  ST_SET workset;
  ST_SET::iterator work_it;

  // construct a workset
  FOREACH_SYMBOL(CURRENT_SYMTAB, st, st_idx) {
    workset.insert(ST_st_idx(st));
  }
  // sequence empty should be processed separately
  // because sequence find assume no empty
  if (sequence->empty()) {
    for (work_it = workset.begin(); 
           work_it != workset.end(); work_it++) {
      list->push_back(*work_it);       
    }
    return; 
  }
  ...
  while (!workset.empty()) {
    for (work_it = workset.begin(); 
      ...
      seq_it = sequence->find(*work_it);
      if (seq_it == sequence->end() ||
          seq_it->second->empty()) {

        list->push_back(*work_it);
        Remove_St_In_Sequence(sequence, *work_it);
        workset.erase(work_it);
...
/*
 * add by mw, the output sequence of st is disinorder
 * use of st in initialization stmt is not defined until the 
 * stmt behind it. we build a sequence here
 * sequence is a map from a st_idx to a st_idx set
 * st_idxs in the set must be defined before the st_idx
 * eon bug5
 */

static void
WN2C_Arrange_St_Sequence(ST_LIST *list)
{
  ST *st;
  ST_IDX st_idx;
  INITO_IDX inito;
  ...
  // sequence = STL_TYPE_ALLOC(ST_MAP);
  sequence = new ST_MAP;
  ...
  FOREACH_SYMBOL(CURRENT_SYMTAB, st, st_idx) {
    if (ST_is_initialized(st) && 
      ...
      // find init item in INITO
      inito = Find_INITO_For_Symbol(st);
      if (inito != 0) {
        // create a set
        // setptr = STL_TYPE_ALLOC(ST_SET);
        setptr = new ST_SET;
        (*sequence)[ST_st_idx(st)] = setptr;
        // find all st used in initialization stmt
        // in the INITV tree
        Find_St_In_Initv(INITO_val(inito), setptr, ST_st_idx(st));
      }
    }
  }

  // Print_Seq(NULL, sequence);
  WN2C_Build_Queue(sequence, list);
}
 #endif
 #ifndef W2C
static void
			  ...
			  ST_LIST *list = NULL)
{
   ...
   // add by mw, use list instead of traversing CURRENT_SYMTAB
   if (list == NULL) {
     list = new ST_LIST;
     FOREACH_SYMBOL(CURRENT_SYMTAB, st, st_idx)
     {
       list->push_back(ST_st_idx(st));
     }
   }
   ...
   for (it = list->begin(); it != list->end(); it++)
   {
      ...
      {
	 tmp_tokens = New_Token_Buffer();
	 // add by mw, remove repeated const initialization in 
	 // WN2C_Append_Symtab_Vars func , we need to append 
	 // weak pragma here, eon bug4
	 BOOL no_comma = FALSE;
			...
			ST_LIST *list = NULL)
{
   ...
   /*XLZHANG: global initialized tokens that contain function pointers
    should be positioned behind that function declarations; otherwise,
    the preprocessor will not be able to find the declaration.*/
    TOKEN_BUFFER global_initialized_tokens;
    global_initialized_tokens = New_Token_Buffer();

   /* FIXME: */
   name_set.Insert("strcmp");
   ...
   for (it = list->begin(); it != list->end(); it++)
   {
     ...
     /* FIXME: */
     //WEI: ignore static or global variables, 
     //since they're handled specially by the initialization script
     //For SCLASS_PSTATIC, we additionally check that the symbol name
     //does not include ".init" (such name is reserved for temp symbols
     //representing array initializers, and need to be outputed)
     bool global = ST_sym_class(st) == CLASS_VAR && 
       ((ST_sclass(st) == SCLASS_PSTATIC &&
         strstr(ST_name(st), ".init") == NULL) ||
     ...
     /*xlzhang*/
     bool is_initialized = ST_is_initialized(st);


     //WEI: for testing, block all extern functions that returns void for now
     bool extern_void_fun = 
       (ST_sym_class(st) == CLASS_FUNC &&
        ST_sclass(st) == SCLASS_EXTERN);

     extern_void_fun = extern_void_fun &&
       (TY_mtype(TY_ret_type(ST_pu_type(st))) != MTYPE_F8);

     global = global || extern_void_fun;
     // gcc012 modified by shihui
     // in case varialbe is declared the same name.
    // if(*it!= lastit &&(ST_export(st)!=EXPORT_LOCAL ||ST_sclass(st) != SCLASS_EXTERN))
     if(*it!= lastit)
     if (!(ST_sym_class(st) == CLASS_FUNC&&Stab_Reserved_St(st) ) ||
           (strcmp(ST_name(st), "errno") == 0 && ST_sclass(st) == SCLASS_EXTERN))
	  ...
	  (ST_sym_class(st) == CLASS_VAR ||
	   ...
	   // gcc012 modified by shihui
     // in case varialbe is declared the same name.
	  !(ST_sym_class(st) == CLASS_FUNC && Stab_Reserved_St(st) )                       &&
	  ...
	  (Stab_External_Def_Linkage(st)                        || 
	   ...
	   BE_ST_w2fc_referenced(st)) ||
          global)
      {
         name_set.Insert(ST_name(st));
	 tmp_tokens = New_Token_Buffer();
        // add by mw, delay output to WN2C_Append_Symtab_Consts func
        // we must put the filter in front of all other branches, eon bug4
	 if (ST_is_weak_symbol(st))
	 {
	    ST2C_weakext_translate(tmp_tokens, st, context);
	 }/*
	 ...
	 else if (global && is_initialized)
	 {
	    ST2C_decl_translate(global_initialized_tokens, st, context);
	    Append_Token_Special(global_initialized_tokens, ';');
            Append_Indented_Newline(global_initialized_tokens,2);
	    continue;
	 } else if (ST_sym_class(st) == CLASS_FUNC && ST_sclass(st) == SCLASS_EXTERN && is_CLIB_Reserved_Func(st))
         {
            Append_Token_String(tmp_tokens,"#ifndef ");
            Append_Token_String(tmp_tokens,ST_name_to_header_file_macro(st));
            ...
            Append_Indented_Newline(tmp_tokens, 1);
            Append_Token_String(tmp_tokens,"#endif");
            Append_Indented_Newline(tmp_tokens, 1);
         } else
         {
       //Append_Token_String(tmp_tokens,"/*wanglei*/");    //wanglei for test
            ST2C_decl_translate(tmp_tokens, st, context);
	//Append_Token_String(tmp_tokens,"/*wanglei*/");   // wanglei for test
            Append_Token_Special(tmp_tokens, ';');
         }
   ...
   /*xlzhang*/
   if (tokens != NULL)
       Append_And_Reclaim_Token_List(tokens, &global_initialized_tokens);
   else
       Write_And_Reclaim_Tokens(W2C_File[W2C_DOTH_FILE],
                                NULL, /* No srcpos map */
                          &global_initialized_tokens);
		      ...
		      CONTEXT      context)
{
   ...
   is_bit_field = (preg_mtype == MTYPE_BS);
   /* Cast the lhs to the type of the preg, and dereference the
    * resultant address.
    */
   Append_Token_String(tmp_tokens, var_name);
   status = WN2C_based_lvalue_wn(tmp_tokens,   /* base variable name */
			      status,         /* tokens represent an lvalue */
			      Stab_Pointer_To(PUINFO_RETURN_TY),
			      Stab_Pointer_To(PUINFO_RETURN_TY),
			                    /* type of base address */
			      preg_ty,      /* expected type of object */
		       ...
		       CONTEXT      context)
{
   ...
   else /* Return through a preg; the usual case */
   {
       ...
       */
      if (preg_mtype == MTYPE_V) /* FIXME: UPC */
      {
	 /* The resultant value from the call is not used anywhere! */
	 return_value_is_used = FALSE;
      }
#if 1 //simd whirl2c
      else if (result_var != NULL && !CALLSITE_in_regs(WN2C_Prev_CallSite))
#else
...
//		 ("Encountered unexpected reference to a return register"));
        if (CALLSITE_in_regs(WN2C_Prev_CallSite)) {
          RETURN_SAVE_NOTSKIP = TRUE;
        }
#endif
	 ...
	 {
           /* FIXME: UPC */
	    status = WN2C_lvalue_st(STID_KIND,
                                    lhs_tokens,
...
#if 1 //simd whirl2c
      else if (result_store != NULL && !CALLSITE_in_regs(WN2C_Prev_CallSite))
#else
...
#if 1 // simd whirl2c
        if (CALLSITE_in_regs(WN2C_Prev_CallSite)
             && (result_var != NULL || result_store != NULL)) {
          RETURN_SAVE_NOTSKIP = TRUE;
        }
#endif
	 TY_IDX preg_ty = 
	 ...
	 if (1 && TY_kind(return_ty) == KIND_STRUCT) {
			   ...
			   CONTEXT context)
{
   ...
   else /* Return through a preg; the usual case */
   {
      ...
      // gcc014 modified by shihui, if return value is a field in a
      // struct. It is not quite correct using WN2C_lvalue_st
      // to the get the field, escpecially if the field is a bit field
      // use result_store exprresion to translate the whirl.
      // must in the form
      //   ldid
      // stid
      if (result_store != NULL &&
          result_var != NULL &&
          ST_sym_class(result_var) != CLASS_PREG &&
          TY_kind(ST_type(result_var)) == KIND_STRUCT) {
      	 /* We have a store into the return register, so just return
      	  * the rhs of the store (should be an STID).
      	  */
      	 Is_True(WN_operator(result_store) == OPR_STID, 
      	 ...
      	 if (!WN2C_assignment_compatible_types(PUINFO_RETURN_TY,
      		                      WN_Tree_Type(WN_kid0(result_store))))
      	 {
      	    // bug vpr/alloc_matrx return type is a compuation expression
      	    WHIRL2C_parenthesize(value_tokens);
      	    WN2C_prepend_cast(value_tokens,
      			      PUINFO_RETURN_TY,
      			      FALSE/*ptr_to_type*/);
      	 }
	 ...
	 else
	 {
	    	if(result_store != NULL ) {
	 		...
	 		if (!WN2C_assignment_compatible_types(PUINFO_RETURN_TY,
		                      	WN_Tree_Type(WN_kid0(result_store)))) {
	    			// vpr002 modifed by shihui
	    			// vpr/alloc_matrx return type is a compuation expression
	    			WHIRL2C_parenthesize(value_tokens);
	    			WN2C_prepend_cast(value_tokens,
			      				PUINFO_RETURN_TY,
			      				FALSE/*ptr_to_type*/);
			     ...
			     CONTEXT       context)
{
   ...
   for (stmt = first; stmt != NULL; stmt = WN_next(stmt))
   {
	 ...
	 // add by mw, insert _GLOBAL__D_main() before any return stmts
	 // in main pu
	 if (CONTEXT_ty_insert_main(context) &&
              WN_operator(stmt) == OPR_RETURN) {
          Append_Indented_Newline(tokens, 1);
          Append_Token_String(tokens, "_GLOBAL__D_main();");
          Append_Indented_Newline(tokens, 1);
	 }
  //  Append_Token_String(tokens, "/*wanglei*/");
	 status = WN2C_translate(tokens, stmt, context);
...
{
  DevWarn(" !!!!!!!!!!   NYI   !!!!!!!!!!!!!\n");
  Append_Token_String(tokens, "__asm __volatile (");
  WN *block0 = WN_kid0(wn);
  WN *block1 = WN_kid1(wn);
  Is_True(WN_operator(block0) == OPR_BLOCK, 
          ("asm stmt kid0 should be a block"));
  //Append_Indented_Newline(tokens, 1);
  ...
  // asm stmts
  char *stmts = strdup(ST_name(WN_st(wn)));
  // remove all '\t' in stmts
  char *p1 = stmts;
  while ((*p1) != '\0') {
    if ((*p1) == '\t') {
  ...
  // split stmts using '\n'
  char *chr = "\n";
  char *str = strtok(stmts, chr);
  while (str != NULL) {
    Append_Token_Special(tokens, '"');
    Append_Token_String(tokens,  str);
    Append_Token_Special(tokens, '\\');
    Append_Token_Special(tokens, 'n');
    Append_Token_Special(tokens, '\\');
    Append_Token_Special(tokens, 't');
    Append_Token_Special(tokens, '"');
    str = strtok(NULL, chr);
    if (str != NULL) {
      Append_Indented_Newline(tokens, 1);
  ...
  if (WN_first(block1) != NULL ||
       WN_kid_count(wn) >= 2  ||
       WN_first(block0) != NULL) {
    WN2C_Stmt_Newline(tokens, CONTEXT_srcpos(context));
    Append_Token_String(tokens, ": ");
  }
  for (WN *stmt = WN_first(block1); stmt != NULL; stmt = WN_next(stmt)) {
    Is_True(WN_operator(stmt) == OPR_PRAGMA, // && 
            // WN_offset(stmt) == WN_PRAGMA_ASM_CONSTRAINT, 
            ("asm output constraint should be pragma list"));
    // output "constraint"
    Append_Token_Special(tokens, '"');
    Append_Token_String(tokens, WN_pragma_asm_constraint(stmt));
    Append_Token_Special(tokens, '"');
    // output operand
    Append_Token_String(tokens, " (");
    INT preg_idx = WN_pragma_asm_copyout_preg(stmt);
    TY_IDX object_ty = PUinfo_Preg_Type(ST_type(WN_st(stmt)), preg_idx);
    WN2C_Append_Preg(tokens, WN_st(stmt), 
                     preg_idx, object_ty, context);
    Append_Token_Special(tokens, ')');
  ...
  // input constaint list
  if (WN_kid_count(wn) >= 2 || WN_first(block0) != NULL) {
    WN2C_Stmt_Newline(tokens, CONTEXT_srcpos(context));
    Append_Token_String(tokens, ": ");
  }
  for (INT i = 2; i < WN_kid_count(wn); i++) {
    WN *asm_input = WN_kid(wn, i);
    Is_True(WN_operator(asm_input) == OPR_ASM_INPUT, 
            ("asm input constraint should be asm input"));
    // input "constraint"
    Append_Token_Special(tokens, '"');
    Append_Token_String(tokens, WN_asm_input_constraint(asm_input));
    Append_Token_Special(tokens, '"');
    // input operand
    Append_Token_String(tokens, " (");
    // CONTEXT_set_needs_lvalue(context);
    (void)WN2C_translate(tokens, WN_kid0(asm_input), context);
    Append_Token_Special(tokens, ')');
  }

  // clobber constraint list
  if (WN_first(block0) != NULL) {
    Append_Indented_Newline(tokens, 1);
    Append_Token_String(tokens, ": ");
  }
  for (WN *stmt = WN_first(block0); stmt != NULL; stmt = WN_next(stmt)) {
    Is_True(0, ("clobber list NYI"));
  }
  Is_True(WN_operator(block1) == OPR_BLOCK, 
          ("asm stmt kid1 should be a block"));
...
WN2C_func_entry(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context)
{
  ...
  if (!ST_keep_name_w2f(WN_st(wn))) {
   ...
   /* initialization
     * add by mw, for translate negative asm preg idx to positive
     * eon bug15
     */
   asm_preg_num = 0;
   for (INT i = 0; i < MAX_ASM_PREG_NUM; i++) {
      neg_to_pos[i] = 0;
   }
...
WN2C_block(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context)
{
   ...
   if (new_func_scope)
   {
	 ...
	 WN2C_Declare_Return_Parameter(PUinfo_local_decls, context);
      ...
      #else
        // add by mw, as for cxx, return parm is declared in func header, 
      // the declare of return parm and return value in func body is of 
      // no need. eon bug11
      if (!src_cplus && PUINFO_RETURN_TO_PARAM)
	 WN2C_Declare_Return_Parameter(PUinfo_local_decls, context);
      else if (!src_cplus && WN2C_Used_Return_Value)
      #endif
...
WN2C_do_loop(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context)
{
   ...
   if (W2C_Emit_Cgtag && loop_info != NULL)
   {
      ...
      #ifdef W2C
      if (Pointer_Mtype == MTYPE_U8)
      Append_Token_String(tokens, Number_as_String(UINTPS(loop_info), "%ull"));
      else
      #endif
...
WN2C_return(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context)
{
   ...
   if (PUINFO_RETURN_TY != (TY_IDX) 0 && 
       ...
       #ifdef W2C
        TY_kind(PUINFO_RETURN_TY) != KIND_STRUCT &&
	#endif
...
WN2C_istore(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context)
{
   ...
   // Context_reset_array_base(context);
     ...
     STAB_OFFSET offt = WN_store_offset(wn);
	  ...
	  for(i=0;i<4;i++)
		...
		if (WN_operator(WN_kid1(wn)) == OPR_ARRAY)
			{
			Append_Token_String(lhs_tokens, "V16I4ARRAY_ASSIGN (");
			WN2C_translate(lhs_tokens, WN_kid0(WN_kid1(wn)), context);
			 for (dim = 0; dim < WN_num_dim(WN_kid1(wn))-1; dim++){
				Append_Token_Special(lhs_tokens, '[');
	 			(void)WN2C_translate(lhs_tokens, WN_array_index(WN_kid1(wn), dim), context);
				 Append_Token_Special(lhs_tokens, ']');
      				}
			 Append_Token_Special(lhs_tokens, ',');
			 (void)WN2C_translate(lhs_tokens, WN_array_index(WN_kid1(wn), dim), context);
       ...
       WN2C_memref_lhs(lhs_tokens, 
		      ...
		      offt+i*4,                /* offset from this address */
		      WN_ty(wn),           /* ref type for stored object */
                      WN_field_id(wn),     /* wn field id */
		      WN_opc_dtype(wn),    /* base-type for stored object */
		      context,
                      is_bit_field);
	//   WN2C_translate(lhs_tokens, WN_kid1(wn),context);
           TY_IDX  expected_addr_ty;
            STATUS  lhs_status;
			
   #if 0
	  expected_addr_ty = WN2C_Get_Treety(WN_kid1(wn));
	      WN2C_lvalue_wn(lhs_tokens,
			       WN_kid1(wn),         /* the base address */
			       expected_addr_ty,     /* the base addr_ty */
			       stored_ty, /* type of object stored */
			       offt+1,
			       context,
                               is_bit_field,
                                 WN_field_id(wn));
   ...
   if (WN_operator(wn) == OPR_STID)
   {
      ...
      WN2C_memref_lhs(lhs_tokens, 
		      ...
		      context,
                      is_bit_field);
   #if 0
   } else if (WN_rtype(wn) == MTYPE_BS) {
     // a special process to bit field, correct bit field process depends on
     // field info, not load/store offset
     UINT32 current_field_id = 0;
     TY2C_FLD_INFO field_info;
     field_info.select_tokens = NULL;
     field_info.found_fld = FLD_HANDLE();

     CONTEXT_reset_top_level_expr(context);
     STATUS status = WN2C_translate(lhs_tokens, WN_kid1(wn), context);

     WHIRL2C_parenthesize(lhs_tokens);
     field_info = TY2C_get_field_info_by_id(TY_pointed(WN_ty(wn)),
                      WN_field_id(wn), current_field_id);

     if (!STATUS_needs_ref(status)) {
       Append_Token_Special(lhs_tokens, '.');
     } else {
       Append_Token_String(lhs_tokens, "->");
     }
     Append_And_Reclaim_Token_List(lhs_tokens, &field_info.select_tokens);

     STAB_OFFSET offt = WN_store_offset(wn);
     Is_True(offt == 0, ("offt != 0 when ISTORE is a bitfield istore"));
...
WN2C_mstore(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context)
{
   ...
   if (WN_operator(WN_kid0(wn)) == OPR_MLOAD)
   {
      ...
      {
      #ifndef W2C
         Is_True(TY_size(stored_ty)==WN_const_val(WN_kid1(WN_kid0(wn))), 
                  ("Copy size is not equal to the struct size in MSTORE"));
	 ...
	 /* Dereference the address into which we are storing, if necessary.
	  */
	 if (STATUS_needs_ref(lhs_status)) {
	    Prepend_Token_Special(lhs_tokens, '*');
            STATUS_reset_needs_ref(lhs_status);
         }
         #endif
...
WN2C_stid(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context)
{
   ...
   else
   {
     ...
     if (strcmp(ST_name(st), "__c") == 0) {
      ...
      WN2C_Append_Assignment(temp_tokens, 
			     &lhs_tokens, /* lhs */
			     WN_kid0(wn), /* rhs */
			     stored_ty,   /* expected type of rhs */
			     context);    /* Get the rhs expression */
			     
 /* special case x = xinit for array initilization */
#ifndef W2C
      if (TY_Is_Array(WN_ty(wn)) &&
          WN_operator(WN_kid0(wn)) == OPR_LDID &&
          TY_Is_Array(WN_ty(WN_kid0(wn)))) {
        TY_IDX array_ty = WN_ty(wn);
        INT32 size = TY_size (WN_ty(wn));
        char bytes[64];
...
WN2C_call(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context)
{
   ...
   if (WN_operator(wn) == OPR_CALL)
   {
	  ...
	  #ifdef W2C 
      return_to_struct = TY_kind(return_ty) == KIND_STRUCT;
	  #endif
      first_arg_idx = (return_to_param? 1 : 0);
      // gcc005
      // the actual argument number may not equal with declaration, 
      // when the declaration is old style example in gcc, the
      // example is gcc sdbout/call plain_type
      // modified by shihui
      TY_IDX declare_ty = ST_pu_type(&St_Table[WN_entry_name(wn)]);
      TYLIST_IDX params = TY_parms(declare_ty);
      if (!TY_is_varargs(declare_ty)) {
        if(Tylist_Table[params] == 0) {
          last_arg_idx = WN_kid_count(wn) - 1;
        }
	#ifdef W2C
       else if (return_to_struct) {
         last_arg_idx = first_arg_idx-1;
         while (Tylist_Table[params] != 0) {
          ...
          last_arg_idx = first_arg_idx-1;
     //     while (Tylist_Table[params] != 0&&WN_kid_count(wn)>last_arg_idx+1) {
     while (Tylist_Table[params] != 0) {
            last_arg_idx++;
            params = TYLIST_next(params);
          }
...
//          }
   ...
   else if (WN_operator(wn) == OPR_ICALL)
   {
      ...
      func_ty = WN_ty(wn);
      // add by shihui, type-casting for function pointer
      TY_IDX func_prt_ty = WN_ty(WN_kid(wn, WN_kid_count(wn) - 1));
      if(TY_Is_Pointer(func_prt_ty) &&
         WN2C_compatible_qualified_types(TY_pointed(func_prt_ty), func_ty)) {
      }
      else {
        WN2C_prepend_cast(call_tokens, func_ty, TRUE);
        WHIRL2C_parenthesize(call_tokens);
      }
      // end
   ...
   // add by mw, return ty converted to a fake parm, func prototype
   // is also changed to suit it, we output parms according to the type
   // of prototype if it exists. For gfecc processing is different with gfec, 
   // so process whirl2c for cxx independent from whirl2c for c. 
   // eon bug11
   if (src_cplus) {
     if (WN_operator(wn) == OPR_CALL) {
       TY_IDX declare_ty = ST_pu_type(&St_Table[WN_entry_name(wn)]);
   ...
   extern BOOL TY_should_convert(const TY_IDX ty);
   for (arg_idx = first_arg_idx; arg_idx <= last_arg_idx; arg_idx++)
   {
      if(WN_operator(WN_kid(wn, arg_idx)) != OPR_PARM)
      	{
      	Append_Token_Special(call_tokens, '0');
	 ...
	 Append_Token_Special(call_tokens, ',');
      ...
      // add by wy, for consistency with Reset_param_list func in ipo_const.cxx
      if (WN_Parm_Dummy(WN_kid(wn, arg_idx))) {
        continue;
      }
      ...
      //
      // vpr001 return type is struct modified by shihui
      if(arg_idx == 0 && TY_kind(return_ty) == KIND_STRUCT) {
        TY_IDX parm0_ty = TY_pointer(return_ty);
        CONTEXT_set_given_lvalue_ty(context, parm0_ty);
      }
      // end
      // add by mw, struct cannot be used as parm directly, it will
      // converted by compiler to struct st*, precise modification
      // should be at frontend. left aside to be improved
      // for time reason, I use src_cplus to restrict the effect to cxx
      // , but c have the same problem. eon bug10
      /*else if (src_cplus &&
             Tylist_Table[param_tylist] != TY_IDX_ZERO &&
             TY_kind(TYLIST_item(Tylist_Table[param_tylist])) == KIND_STRUCT) {
         CONTEXT_set_given_lvalue_ty(context,
                  Make_Pointer_Type(TYLIST_item(Tylist_Table[param_tylist])));

      }*/
      else if (Tylist_Table[param_tylist] != TY_IDX_ZERO)
	 CONTEXT_set_given_lvalue_ty(context,
				     TYLIST_item(Tylist_Table[param_tylist]));
      else
        /* FIXME: UPC (TY_IDX_ZERO) */
	 CONTEXT_set_given_lvalue_ty(context,
				     TY_IDX_ZERO);
...
WN2C_iload(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context)
{
   ...
   if (WN_operator(wn) == OPR_LDID)
   {
     ...
     WHIRL2C_parenthesize(expr_tokens);
     field_info = TY2C_get_field_info_by_id(WN_ty(wn),
                      WN_field_id(wn), current_field_id);
     ...
     STAB_OFFSET offt = WN_load_offset(wn);
     Is_True(offt == 0, ("offt != 0 when ILOAD is a bitfield iload"));
   #endif
     ...
     // m21 when addr_ty is pointer of struct type and object_ty is field type
     //     incompatible type in WN2C_memref_lhs is meaningful. the object type
     //     is constructed in WN2C_MemAccess_Type
     #if 0
     /* FIXME: */
     if (WN_field_id(wn) > 0 && TY_Is_Pointer(actual_ty))
      WN2C_memref_lhs(expr_tokens, 
		      &loaded_ty,
		      WN_kid0(wn),         /* address */
		      offt,                /* offset from this address */
                      actual_ty,           /* ref type for loaded object */
		      TY_pointed(actual_ty), /* type for loaded object */
		      WN_opc_dtype(wn),    /* base-type for stored object */
		      context);
     else
     #endif
      WN2C_memref_lhs(expr_tokens, 
		      ...
		      WN_kid0(wn),         /* address */
		      offt,                /* offset from this address */
                      Make_Pointer_Type(WN_ty(wn)),       /* ref type for loaded object */
		      WN_field_id(wn),     /* wn field id */
		      WN_opc_dtype(wn),    /* base-type for stored object */
		      context,
                      is_bit_field);
   ...
   MTYPE desc = WN_desc(wn);
   MTYPE kid_point_to =  TY_mtype (loaded_ty);
   if (desc != kid_point_to )  {
   	if ( !is_bit_field) { // convert to bits is strange to use explicit CVT
   	    WHIRL2C_parenthesize(expr_tokens);
	    WN2C_prepend_cast(expr_tokens, 
		     Stab_Mtype_To_Ty(WN_desc(wn)), 
		     FALSE/*pointer_to_type*/);
   	}
   }
   
   #if 0
   TY_IDX expected_ty = WN2C_Get_Treety(wn);

   /* Cast the resultant value to the expected result type if 
    * different from the type of value loaded.
    */
   if ((expected_ty != 0) && !WN2C_arithmetic_compatible_types(loaded_ty, expected_ty)) {
      Is_True(TY_Is_Pointer(expected_ty), ("expected_ty is not a pointer type"));
      WN2C_prepend_cast(expr_tokens, expected_ty, FALSE/*pointer_to_type*/);
   }
   #endif
...
WN2C_mload(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context)
{
   ...
   /* Dereference the address from which we are loading, if necessary */
   if (STATUS_needs_ref(load_status)) {
      Prepend_Token_Special(expr_tokens, '*');
      STATUS_reset_needs_ref(load_status);
   }
   #endif
...
WN2C_array(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context)
{
   ...
   if (num_dim > 1 && WN_operator(base_wn) == OPR_LDA &&
     ...
     WN* offset = WN_Mpy(Integer_type, WN_COPY_Tree(WN_array_index(wn, 0)), WN_Intconst(Integer_type, WN_element_size(wn)));
     for (WN* tmp = WN_kid0(wn); WN_operator(tmp) == OPR_ARRAY; tmp = WN_kid0(tmp)) {
       offset = WN_Add(Integer_type, offset, 
                       WN_Mpy(Integer_type, WN_COPY_Tree(WN_array_index(tmp, 0)), 
   ...
   if (WN_num_dim(wn) >= 2) {
     ...
     INT32 num_dim = WN_num_dim (wn);
     WN* tmp;
  
     for (INT32 i = 0; i < num_dim; i++) {
       tmp = WN_Create(OPR_ARRAY, Pointer_Mtype, MTYPE_V, 3);
       WN_array_base(tmp) = base;
       WN_array_index(tmp, 0) = WN_array_index (wn, i);
       WN_array_dim(tmp, 0) = WN_array_dim (wn, i);
       base = tmp;
     }

     return WN2C_array(tokens, tmp, context);
   }
#endif
   ...
   CONTEXT_set_array_base(context);
   base_addr_status = WN2C_translate(tmp_tokens, WN_kid0(wn), context);
   context = save;

   if (WN_operator(WN_kid0(wn)) == OPR_LDA && 
         STATUS_needs_addr(base_addr_status)) {
     Prepend_Token_Special(tmp_tokens, '&');
     STATUS_reset_needs_addr(base_addr_status); 
   }

   // m27.   ARRAY 400   ==> type got from bottom incompatile with array elem size
   //        INTCONST 10
   //        LDID i
   //      ARRAY 4
   //    F4F4ILOAD
   ...
   if (WN_operator(WN_kid0(wn)) == OPR_ARRAY) {
     TY_IDX expected_ty = WN2C_Get_Treety(WN_kid0(wn));     
     TY_IDX tree_ty = WN_Tree_Type(WN_kid0(wn));
     if (!WN2C_unparser_compatible_types(tree_ty, expected_ty)) {
       TY_IDX elemty = TY_pointed(WN2C_Get_Treety(wn));
       TY_IDX expected_ty = Get_Array_Tree_Ty((WN *)wn, elemty, TRUE);
       WN2C_prepend_cast(tmp_tokens, expected_ty, FALSE);
     }
   //} else if (WN_operator(WN_kid0(wn)) == OPR_LDID) {
     ...
     TY_IDX tree_ty = WN_Tree_Type(WN_kid0(wn)); 
     if (!WN2C_unparser_compatible_types(tree_ty, expected_ty)) {
	// 	WN2C_unparser_compatible_types(tree_ty, expected_ty);
       WN2C_prepend_cast(tmp_tokens, expected_ty, FALSE);
     }
   }
   #if 0 
   ...
   // by mw, not certain if the following code is useless in new framework 
   else
   {
      treat_ptr_as_array = FALSE;
   ...
   // m2. add following to cooperate with the STATUS_reset_lvalue in WN2C_lda/ldid
   # if 0
   if (STATUS_needs_ref(base_addr_status)) {
     Prepend_Token_Special(tmp_tokens, '*');
     STATUS_reset_needs_ref(base_addr_status);
     WHIRL2C_parenthesize(tmp_tokens);
   }
   #endif

   // add by mw, assume array always return lvalue
   // STATUS_set_lvalue(return_status);           /* indexed is lvalue */
   WHIRL2C_parenthesize(tmp_tokens);
   Append_And_Reclaim_Token_List(tokens, &tmp_tokens);
   // CONTEXT_reset_needs_lvalue(context);
   ...
   // m6. I believe array can all be normalized
   #if 0 
   if (treat_ptr_as_array ||
       TY_Is_Pointer(base_ty) && 
   ...
   if (WN_operator(WN_array_index(wn, dim)) == OPR_LDA ){
   	 Append_Token_String(tokens, "( _UINT64 )");
        //WN2C_prepend_cast(tokens, Stab_Mtype_To_Ty(MTYPE_I8), FALSE);
   }
	 #endif
   ...
   STATUS_set_no_direct_cast(return_status);
   STATUS_reset_needs_ref(return_status);
   STATUS_reset_needs_addr(return_status);
   return return_status;
} /* WN2C_array */
   #else
static STATUS 
...
WN2C_cvt(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context)
{
  ...
  if (STATUS_needs_addr(status) && (WN_operator(wn) == OPR_LDA)) {
   ...
   /*xlzhang: change intptr_t to int*, because the pathcc compiler can't recognize type intptr_t.*/     
       //Prepend_Token_String(expr_tokens, "(intptr_t)");
   //    Prepend_Token_String(expr_tokens, "(int *)");
   //   fixed by axm
       Prepend_Token_String(expr_tokens, "(unsigned long)");   
...
WN2C_ldid(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context)
{
   ...
   if (ST_sym_class(WN_st(wn)) == CLASS_PREG)
   {
	...
	WN2C_Append_Preg(expr_tokens, 
     ...
     ST2C_use_translate(expr_tokens, st, context);
     WHIRL2C_parenthesize(expr_tokens);
     field_info = TY2C_get_field_info_by_id(ST_type(st), 
                      WN_field_id(wn), current_field_id, fld_ofst);
     Append_Token_Special(expr_tokens, '.');
     Append_And_Reclaim_Token_List(expr_tokens, &field_info.select_tokens);

     Is_True(addr_offset == 0, ("addr_offset != 0 when LDID is a bitfield ldid")); 
   } else {
     ...
     /* if (TY_is_union(WN_ty(wn)) && WN_field_id(wn) > 0) {
      ...
      // m13. change WN2C_SymAccess_Type to WN2C_MemAccess_Type
      /* Get the lhs, preferably as an lvalue, but possibly as an address.
       */
      #if 0
      WN2C_SymAccess_Type(&base_addr_ty,
      ...
      #endif
      object_ty = WN_ty(wn);
      TY_IDX wn_st_type = ST_type(WN_st(wn));
      #if 0
      object_ty =
            WN2C_MemAccess_Type(
      ...
      /* Get the lvalue or address of the data to be loaded */
      load_status = WN2C_lvalue_st(LDID_KIND,
                                   expr_tokens,
				   WN_st(wn),    /* base symbol loaded from */
				   wn_st_type,   /* address loaded from */
				   object_ty,    /* type of object loaded */
				   addr_offset,
				   context,
                                   WN_field_id(wn));
    ...
    */
   TY_IDX expected_ty = WN2C_Get_Treety(wn); 
   if ((expected_ty != 0) && !WN2C_unparser_compatible_types(object_ty, expected_ty))
   {
     Is_True(TY_Is_Pointer(expected_ty), ("expected_ty is not a pointer type"));
     WN2C_prepend_cast(expr_tokens, WN_ty(wn), FALSE/*pointer_to_type*/);

      // m18. nullify the following code
      #if 0
      else
      {
	 /* Assign the expr_tokens to a temporary, cast the address of the
   ...
   // m7. I believe ldid return all lvalue, because no '&' is added here
   //     even if added, a '*' is also added.
   STATUS status = EMPTY_STATUS;
   STATUS_set_needs_ref(status);
   #if 0
   if (CONTEXT_needs_lvalue(context)) {
     STATUS_set_lvalue(status);
   }
   #endif
   return status;
...
WN2C_lda(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context)
{
   ...
   {
      Is_True(TY_Is_Pointer(WN_ty(wn)), ("WN_ty(wn) should be a pointer type"));
	 ...
	 object_ty = Stab_Mtype_To_Ty(MTYPE_U1);
      ...
      // when st is array type, the lda wn_ty is pointer of array type, which
      // may make the expr very complex
      #if 0
      object_ty = WN2C_Get_Treety(wn);
      if (object_ty ==  0) {
        object_ty = WN_ty(wn);
        // a very special code, to process LDA st T<B>, if st is of type array A, 
        // then T will be A *, which is not compatible with the object_ty of LDA,
        // which should be of type T<(elemty of A) *>
        if (TY_Is_Pointer(object_ty) && TY_Is_Array(TY_pointed(object_ty))) {
          object_ty = Make_Pointer_Type(TY_etype(TY_pointed(object_ty)));
        }
        // object_ty = wn_st_type;
      }
      #endif
      object_ty = WN_Tree_Type(wn);
	 ...
	 if (TY_kind(wn_st_type) == KIND_ARRAY &&
	   ...
	   prefered_ty = TY_etype(wn_st_type);
         ...
         #if 0
         if (TY_Is_Structured(wn_st_type)) {
           if (WN_field_id(wn) > 0) {
             st_ty = Get_Field_Type(wn_st_type, WN_field_id(wn));
           } else if (lda_offset != 0) {
             st_ty = WN2C_MemAccess_Type(
                           st_type,                  /* base_type */
                           prefered_ty,                  /* preferred type */
                           TY_mtype(prefered_ty),         /* required mtype */
                           lda_offset);                  /* offset from base */
           }
         }
        

         // if TY_Is_Array(st_ty), we speculate st_ty to be array elemty
         // better have some context info to indicate the parent is array or not
         if (TY_Is_Array(st_ty)) {
           st_ty = TY_etype(st_ty);
         }
         #endif

      // m12. WN_ty(wn) should be a pointer type, so the branch is nonsense
      #if 0
      else
      {
         object_ty = ST_sym_class(WN_st(wn)) == CLASS_FUNC ?
      ...
      // speculate on the addr ty of st, if the speculation is wrong, 
      // the object_ty will preserve the correctness.
      #if 0
      TY_IDX addr_ty = Stab_Pointer_To(ST_sym_class(WN_st(wn)) == CLASS_FUNC ?
                                   ST_pu_type(WN_st(wn)) : ST_type(WN_st(wn))),
      if (TY_Is_Structured(TY_pointed(addr_ty))) {
        if (WN_field_id(wn) > 0) {
          object_ty = Get_Field_Type(prefered_ty, WN_field_id(wn));
        } else if (lda_offset != 0) {
          object_ty = WN2C_MemAccess_Type(
                        addr_ty,                      /* base_type */
                        addr_ty,                      /* preferred type */
                        TY_mtype(wn_st_type),         /* required mtype */
                        lda_offset);                  /* offset from base */
        }
      }      
      ...
      expr_tokens = New_Token_Buffer();
      lda_status = 
	 WN2C_lvalue_st(LDA_KIND,
                        expr_tokens, 
			WN_st(wn),  /* base st and the addr ty */
                        wn_st_type,    /* proper ty get from st */
			object_ty,  /* type addressed */
			lda_offset, 
			context);
      ...
      /* Convert an lvalue into an address value, if necessary.*/
      // m3. WN2C_arithmetic_compatible_types ==> WN2C_unparser_compatible_types
      #if 0
      TY_IDX expected_ty = WN2C_Get_Treety(wn);
      if ((expected_ty != 0) && !WN2C_unparser_compatible_types(object_ty, expected_ty)) {
        Is_True(TY_Is_Pointer(expected_ty), ("expected_ty is not a pointer type"));
        WN2C_prepend_cast(expr_tokens, expected_ty, FALSE/*pointer_to_type*/);
        // m11. WN2C_unparser_compatible_types(object_ty, prefered_ty) == FALSE
        //      means correct structure field is found, the correct type is 
        //      found in WN2C_MemAccess_Type 
        // STATUS_set_lvalue(lda_status); /* Not returning an lvalue */
	 ...
	 STATUS_reset_lvalue(lda_status); /* Not returning an lvalue */
      ...
      #endif
      if (STATUS_needs_addr(lda_status)) {
        /* Is_True(TY_Is_Pointer(object_ty), 
                 ("object_ty of OPR_LDA should be pointer type")); */
        // if TY_Is_Structured(TY_pointed(object_ty)), we postpone to add
        // &. In this way, we translate st.a to (st).a not (&st)->a
        if (!TY_Is_Pointer(object_ty) || !TY_Is_Structured(TY_pointed(object_ty))) {
	  Prepend_Token_Special(expr_tokens, '&');
	  STATUS_reset_needs_addr(lda_status); /* We not returning an lvalue */
        } else {
         // Is_True(0, ("catch the case '&' output postpone in WN2C_lda"));
        }
      }
   }
   Append_And_Reclaim_Token_List(tokens, &expr_tokens);
   STATUS_set_needs_ref(lda_status); /* We not returning an lvalue */
...
WN2C_const(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context)
{
   ...
   if (!TY_Is_Complex(WN_Tree_Type(wn)))
	  ...
	  if(WN_rtype(wn)==MTYPE_V16I4&&TCON_ty(STC_val(WN_st(wn)))==MTYPE_I4)
	  	{
	  	Append_Token_String(tokens,"(V16I4){");
	   ...
	   else if(WN_rtype(wn)==MTYPE_V16I8&&TCON_ty(STC_val(WN_st(wn)))==MTYPE_I8)
	  	{
	  	Append_Token_String(tokens, " * 2");
	  	}
	   else
	   	{
	   	TCON2C_translate(tokens, STC_val(WN_st(wn)));
...
static STATUS 
WN2C_reduce_add(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context)
{
   Is_True(WN_operator(wn) == OPR_REDUCE_ADD,
	   ("Invalid operator for WN2_reduce_add()"));
   Append_Token_String(tokens, "I4V16I4REDUCE_ADD(");
   WN2C_translate(tokens, WN_kid0(wn), context);
   Append_Token_String(tokens, ")");
...
static STATUS 
WN2C_replicate(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context)
{
   Is_True(WN_operator(wn) == OPR_REPLICATE,
	   ("Invalid operator for WN2_replicate()"));
   Append_Token_String(tokens, "(V16I4) {");
   WN2C_translate(tokens, WN_kid0(wn), context);
   Append_Token_String(tokens, ",");
     WN2C_translate(tokens, WN_kid0(wn), context);
...
WN2C_parm(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context)
{
   ...
   if (WN2C_unparser_compatible_types(parm_ty, WN_Tree_Type(WN_kid0(wn)))) {
      status = WN2C_translate(tokens, WN_kid0(wn), context);

      if (STATUS_needs_addr(status) && (WN_operator(WN_kid0(wn)) == OPR_LDA)) {
        Prepend_Token_Special(tokens, '&');
        STATUS_reset_needs_addr(status);
        STATUS_reset_needs_point(status);
      }
   } else {
     if (!volatile_arg && WN2C_unparser_compatible_types(parm_ty, arg_ty) &&
         !WN2C_int_downcast(arg_ty, parm_ty)) {
       status = WN2C_translate(tokens, WN_kid0(wn), context);
       if (STATUS_needs_addr(status) && (WN_operator(WN_kid0(wn)) == OPR_LDA)) {
         Prepend_Token_Special(tokens, '&');
       ...
       parm_tokens = New_Token_Buffer();
       CONTEXT_reset_top_level_expr(context);
       status = WN2C_translate(parm_tokens, WN_kid0(wn), context);
       ...
       // A very ugly walkaround, it should be modified in the future.
       // although perfect type merging in IPA is still hard to be achieved
       // because of empty structure generated by front end, it should be achieved
       // sometime
       if( TY_Is_Structured(arg_ty) &&
           TY_Is_Structured(parm_ty))  {
          DevWarn("arg and parm ty are both structure and incompatible");
          Is_True(WN_operator(WN_kid0(wn)) == OPR_LDID, 
                  ("arg and parm ty are both structure, OP should be LDID"));
          WHIRL2C_parenthesize(parm_tokens);
          Prepend_Token_Special(parm_tokens, '&');
          WN2C_prepend_cast(parm_tokens, parm_ty, TRUE);
          Prepend_Token_Special(parm_tokens, '*');
          WHIRL2C_parenthesize(parm_tokens);
       }else if( TY_Is_Scalar(arg_ty) &&
           TY_Is_Structured(parm_ty))
       	{
...
WN2C_extract_bits(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context)
{
  ...
  UINT  of = WN_bit_offset(wn);
   // simd whirl2c twolf bug3
/*begin modifying by xlzhang 06-9-27*/
  MTYPE from_signed_to_unsigned = (wn->common).rtype;
  char *TokenType;
  switch (from_signed_to_unsigned){
     case MTYPE_I1:from_signed_to_unsigned = MTYPE_U1;TokenType="(_INT8)";break;
     case MTYPE_I2:from_signed_to_unsigned = MTYPE_U2;TokenType="(_INT16)";break;
     case MTYPE_I4:from_signed_to_unsigned = MTYPE_U4;TokenType="(_INT32)";break;
     case MTYPE_I8:from_signed_to_unsigned = MTYPE_U8;TokenType="(_INT64)";break;
  }
  //UINT  sz = (1 << WN_bit_size(wn)) - 1;
  long long sz=1;
  for (int i=1;i<=WN_bit_size(wn);i++) sz<<=1;
  sz-=1;
/*end modifying by xlzhang 06-9-27*/
...
//begin modified by xlzhang 06.9.26
  Append_Token_Special(tokens,'(');
  if (of!=0){
  Append_Token_Special(tokens,'(');
  }
  ...
  if (WN_operator_is(WN_kid0(wn),OPR_LDID) && TY_Is_Pointer(ST_type(WN_st(WN_kid0(wn))))) Append_Token_String(tokens,TokenType);
  status = WN2C_translate(tokens,WN_kid0(wn),context);
...
//end modified by xlzhang 06.9.26

  Append_Token_Special(tokens, ')');
  /* "& 0x" and "f..." must appear at the same time */
  // simd whirl2c twolf bug3
/*modifying by xlzhang 06-9-27*/
  if (sz > 0) {
    Append_Token_Special(tokens, '&');
    //TCON2C_translate(tokens,
    //               Host_To_Targ(MTYPE_I4, sz));
    TCON2C_translate(tokens,Host_To_Targ(from_signed_to_unsigned,sz));
  } 
/*end modifying by xlzhan 06-9-27*/
...
//begin modified by xlzhang 06.9.26
  if (MTYPE_is_signed((wn->common).rtype)){
    Prepend_Token_Special(tokens,'(');
    Append_Token_Special(tokens,')');
    Append_Token_Special(tokens,'|');
    ...
    long long test_high_bit=1;//sizeof longlong is 8
    //printf("sizeof unsigned long %d,sizeof long %d,sizeof int%d,sizeof longlong %d,sizeof uint %d\n",sizeof(unsigned long),sizeof(long),sizeof(int),sizeof(long long),sizeof(UINT));

    for (int i=1;i<(WN_bit_size(wn)+WN_bit_offset(wn));i++) test_high_bit <<= 1;
    ...
    WN2C_translate(tokens,WN_kid0(wn),context);
    Append_Token_Special(tokens,'&');
    TCON2C_translate(tokens,Host_To_Targ(/*(wn->common).rtype*/from_signed_to_unsigned, test_high_bit));

    Append_Token_String(tokens,") ? ");  

    long long high_bit_or=0;
    for (int j=0;j<MTYPE_bit_size(wn->common.rtype) - WN_bit_size(wn) - WN_bit_offset(wn);j++) high_bit_or=(high_bit_or << 1) + 1;
    high_bit_or = (high_bit_or * test_high_bit) << 1 ;
    TCON2C_translate(tokens,Host_To_Targ(/*(wn->common).rtype*/from_signed_to_unsigned, high_bit_or));

    Append_Token_Special(tokens,':');
    TCON2C_translate(tokens,Host_To_Targ(/*(wn->common).rtype*/from_signed_to_unsigned, 0));    
...
WN2C_compose_bits(TOKEN_BUFFER tokens, const WN *wn, CONTEXT context)
{
  ...
  /* "& 0x" and "f..." must appear as the same time */
  if (sz > 0) {
  ...
  Append_Token_Special(tokens,'(');
  TCON2C_translate(tokens,
                   Host_To_Targ(MTYPE_I4,mask));  
  Append_Token_Special(tokens,'&');
  status1 = WN2C_translate(tokens, WN_kid0(wn), context);
  Append_Token_Special(tokens,')');
  Append_Token_Special(tokens,')');
...
WN2C_translate_file_scope_defs(CONTEXT context)
{
   ...
   /*
    * add by mw, build a sequence for st decl of def, eon bug5
    */
   ST_LIST list;
   ST_LIST *listptr;
   // if (src_cplus) {
     listptr = &list;
     WN2C_Arrange_St_Sequence(listptr);
   ...
   Write_String(W2C_File[W2C_DOTH_FILE], NULL/* No srcpos map */,
		"/* File-level vars and routines and constants */\n");
                ...
                BOOL         is_bit_field)
{
   ...
   */
   ...
   // m1. no base_ty is needed, only addr_ty is needed in WN2C_based_lvalue_wn
   // base_ty = WN_Tree_Type(lhs);
   // if (!TY_Is_Pointer(base_ty))
   *memref_typ = TY_pointed(memref_addr_ty);

   if (!is_bit_field && TY_Is_Structured(*memref_typ)) {
     if (field_id != 0) {
        UINT cur_field_id = 0;
        FLD_HANDLE fld = FLD_get_to_field(*memref_typ, 
                                            field_id, 
                                            cur_field_id);
        *memref_typ = FLD_type(fld);
      } else if (memref_ofst != 0) {
        // if field id != 0, and ofst != 0, then
        // ofst cannot be the ofst of field in struct
        // so the objty is keep as *memref_typ 
        ;
      }
    }
#if 0 /* FIXME: UPC */   
   ...
   expected_addr_ty = WN2C_Get_Treety(lhs);
              ...
              UINT         field_id)
{
   ...
   if (ST_sym_class(lhs_st) == CLASS_PREG)
   {
     ...
     ST2C_use_translate(tokens, lhs_st, context);
     WHIRL2C_parenthesize(tokens);
     field_info = TY2C_get_field_info_by_id(ST_type(lhs_st), 
                      field_id, current_field_id, fld_ofst);
     Append_Token_Special(tokens, '.');
     Append_And_Reclaim_Token_List(tokens, &field_info.select_tokens);

     *stored_typ = (field_id > 0) ? Get_Field_Type(stid_ty, field_id) : stid_ty;
     Is_True(stid_ofst == 0, ("stid_ofst != 0 when STID is a bitfield stid")); 
   } else {
      /* Get the lhs, preferably as an lvalue, but possibly as an 
       * address.
       */
      // m15. change WN2C_SymAccess_Type to WN2C_MemAccess_Type
      #if 0
      WN2C_SymAccess_Type(&base_ty,
			  stored_typ,
			  ST_type(lhs_st), /* base_type */
			  ...
			  stid_ofst);      /* offset from base */
      #endif
      TY_IDX wn_st_type = ST_type(lhs_st);
      ...
      TY_IDX object_ty = stid_ty;
      if (TY_Is_Structured(object_ty)) {
        object_ty = (field_id > 0) ? Get_Field_Type(object_ty, field_id) : object_ty;
      }

      #if 0
      if (TY_Is_Structured(prefered_ty)) {
        if (field_id > 0) {
          object_ty = Get_Field_Type(prefered_ty, field_id);
        } else if (stid_ofst != 0) {
          object_ty = WN2C_MemAccess_Type(
                        wn_st_type,                   /* base_type */
                        prefered_ty,                  /* preferred type */
      ...
      *stored_typ = object_ty;
      lhs_status = WN2C_lvalue_st(STID_KIND,
                                  tokens,
				  lhs_st,      /* base stored into */
				  wn_st_type,     /* base ref stored into */
				  object_ty,   /* type of object stored */
				  stid_ofst,
				  context,
                                  field_id);
