...
/*---------------------------------------------------------------------*/
#ifdef W2C
extern BOOL   whirl2c_include_stdarg;
// add by mw, indicate source is cxx
...
ST2C_basic_decl(TOKEN_BUFFER tokens, const ST *st, CONTEXT context)
{
  ...
  Append_Token_String(decl_tokens, 
      W2CF_Symtab_Nameof_St(st));    /* name */
  //Append_Token_String(decl_tokens, 
  //    "/*wanglei*/");    /* name */        //wanglei  if you see , please delete 
  ...
  if (TY_kind(ty) == KIND_ARRAY &&
      Stab_Array_Has_Dynamic_Bounds(ty))
      CONTEXT_set_incomplete_ty2c(context);
  ...
  if (!Stab_No_Linkage(st))
  {
    ...
    if(W2C_Emit_Gsymtab)
    {
      //added by wy , to add "extern" to the  variable "environ"  in symtab.w2c.h
      if(strncmp(ST_name(st), "environ", 7) == 0)
        Prepend_Token_String(decl_tokens, "extern"); 
    }
    else if (ST_sym_class(st) == CLASS_FUNC &&
	PU_is_inline_function(Pu_Table[ST_pu(st)]) &&
	strcmp(W2CF_Symtab_Nameof_St(st), "main") != 0)
    {
      Prepend_Token_String(decl_tokens, " __inline");
      if (ST_export(st) == EXPORT_LOCAL && !W2C_Emit_Gsymtab) {
        Prepend_Token_String(decl_tokens, "static");
      }
    }
    /* UPC */
    ...
    {
    if (Find_INITO_For_Symbol(st)==0&&!ST_init_value_zero(st)&&ST_export(st)!=EXPORT_LOCAL_INTERNAL&&ST_export(st)!=EXPORT_LOCAL)
      Prepend_Token_String(decl_tokens, "extern");
    }
...
ST2C_decl_var(TOKEN_BUFFER tokens, const ST *st, CONTEXT context)
{
   ...
   if (ST_is_initialized(st) && !Stab_No_Linkage(st)) /* initialize */
   {
      ...
      // add by mw, for the construction and deconstruction of global objects 
      // is done before and after main, it should be put into special sections.
      // eon bug7
      if (src_cplus &&
        ...
        STR_IDX str_idx = Find_Section_Name_For_ST(st);
		
        if (strcmp(&Str_Table[str_idx], ".ctors") == 0) {
          Append_Token_String(tokens, "__attribute__((section(\".ctors\")))");		
        } else if (strcmp(&Str_Table[str_idx], ".dtors") == 0) {
          Append_Token_String(tokens, "__attribute__((section(\".dtors\")))");		
        }
      }
...
#ifdef W2C
 const char* func_name = W2CF_Symtab_Nameof_St(st);
   // gcc004 modified by shihui, if call is va_start
   // no need to declare it in w2c.h
   if(whirl2c_include_stdarg && 
      strcmp(func_name, "va_start"/*INTRN_rt_name(INTRN_VA_START)*/) == 0) {
     return;
   }
   #endif
		 ...
		 CONTEXT       context)
{
    ...
    */
   first_param = (PUINFO_RETURN_TO_PARAM? 1 : 0);
   // add by mw, as for cxx source, if return to param, the 
   // prototype of func have been changed to add a fake param
   // I want to output all the params to coincide with its 
   // callsite and its func entry. eon bug11
   if (src_cplus)
     first_param = 0;
   ...
   // modified for shihui
   // if function is declared in old style and has other prototype
   // which has less paramters, the parms in declaration will be promoted to I4
   // like 
   // extern void foo();              // prototype
   // extern void foo(a, b)           // old style declaration
   //          int a; char b;
   //
   // in w2c.h the prototype will be extern void foo(int a, int b)
   // which is confilict with the one in w2c.c
   // then has_prototype should be set false 
   // first checking if prototype is compatible with the header
   TY_IDX prototype = ST_pu_type(st);
   TYLIST_IDX proto_params = TY_parms(prototype);
   for (param = first_param; params[param] != NULL; param++) {
     TY_IDX param_ty_idx = ST_type(params[param]);
     TY_IDX prototype_param_ty_idx = TYLIST_item(Tylist_Table[proto_params]);
     if(!Stab_Identical_Types(param_ty_idx, prototype_param_ty_idx, TRUE, TRUE, FALSE)) {
       has_prototype = FALSE;
       break;
     }
     proto_params = TYLIST_next(proto_params);
   }
   // end shihui

   // add by mw, c++ is translated to c, so if the prototype is not the same 
   // with func header, we remove the params prototype in the func decl. 
   // eon bug10, eon bug13
   // totally wrong, it will result in run time error because actuals and formals
   // doesn't match
   /*proto_params = TY_parms(prototype);
   if ((Tylist_Table[proto_params] != TY_IDX_ZERO) && src_cplus && has_prototype) {
     for (param = first_param; params[param] != NULL; param++) {
	TY_IDX param_ty_idx = ST_type(params[param]);
       TY_IDX prototype_param_ty_idx = TYLIST_item(Tylist_Table[proto_params]);
       INT flag = TY_CONST | TY_VOLATILE | TY_RESTRICT;
	if ((param_ty_idx & flag) != (prototype_param_ty_idx & flag)) {
         has_prototype = FALSE;
       }
       if (TY_kind(param_ty_idx) == KIND_POINTER && 
            TY_kind(prototype_param_ty_idx) == KIND_POINTER &&
            (TY_pointed(param_ty_idx) & flag) != (TY_pointed(prototype_param_ty_idx) & flag)) {
         has_prototype = FALSE;
       }
     }
   }
   if (src_cplus && has_prototype == FALSE) {
     Clear_TY_has_prototype(funtype);
   } */
...
if (!has_prototype)
   ...
   {  
	 ...
	 if (params[param+1] != NULL)
      ...
      // WEI:  If a struct appears in the function return type,
      // it must be declared as incomplete
      if (1) {
	CONTEXT_set_incomplete_ty2c(context);
      }
      ...
      CONTEXT_set_const(context);
      /*if (src_cplus) {
        // add by mw, remove all the const qualifier, any problem?
        CONTEXT_reset_const(context);	  
      }*/
	  
      for (param = first_param; params[param] != NULL; param++)
      {
	  // Huo Wei. For thunk, it can not be const	
	  // judged by name .arg see wfe_start_function
	  CONTEXT saved_context = context;
	  if(!PUINFO_RETURN_TO_PARAM || param != first_param ) // Not for fake first param
	  	if(!strncmp(ST_name(params[param]),".arg",4))
				CONTEXT_reset_const(context);
	  // end
   ...
   else // (has_prototype)
   {
	 ...
	 if (params[param+1] != NULL)
	    Append_Token_Special(header_tokens, ',');
	  // Huo Wei. For fake first params or thunk, set the context back	
	  // judged by name .arg see wfe_start_function
      ...
      {
      // add by Huo Wei
      // There may be zero iteration of above FOR
   ...
   // add by mw, main should not be inlined;
   // if st is weak and it has inline and extern flag at the same time,
   // there is a conflict between xlocal and weak, because pathcc will 
   // convert xpreemtible to xlocal when inline and extern flags coexist. 
   // I temporarily remove the inline flag to walkaround it, more pretty
   // modification will be done when I have better knowledge about it
   // eon bug6
   if (PU_is_inline_function(Pu_Table[ST_pu(st)]) && 
   	 strcmp(W2CF_Symtab_Nameof_St(st), "main") && 
   	 !ST_is_weak_symbol(st) &&
   	 (ST_export(st) == EXPORT_LOCAL ||
   	  ST_export(st) == EXPORT_LOCAL_INTERNAL 
    // fix for open64 for it treats extern inline as EXPORT_PREEMPRIBLE
...
#else
	     Prepend_Token_String(header_tokens, " __inline");
	    if(strcmp(W2CF_Symtab_Nameof_St(st), "gnu_dev_major") == 0||
	  	strcmp(W2CF_Symtab_Nameof_St(st), "gnu_dev_minor") == 0||
	  	strcmp(W2CF_Symtab_Nameof_St(st), "gnu_dev_makedev") == 0)
	  	{
	  	  Prepend_Token_String(header_tokens, "static");
	  	}
#endif
   ...
   if (ST_export(st) == EXPORT_LOCAL)
     ...
     //so we look at its export scopt instead.  See bug 476. 
      Prepend_Token_String(header_tokens, "static");
#ifdef W2C
   else if (!(PU_is_inline_function(Pu_Table[ST_pu(st)]) && 
   	 strcmp(W2CF_Symtab_Nameof_St(st), "main") ))
#else
	 else
#endif
