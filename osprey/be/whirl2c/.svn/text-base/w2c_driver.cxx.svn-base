...
#ifdef W2C
//addad by wy
BOOL W2C_Emit_Gsymtab = CLIST_emit_gsymtab;
#endif
...
W2C_Enter_Global_Symbols(void)
{
   ...
   {
#ifdef W2C
     if ((ST_sclass(st) == SCLASS_FSTATIC) || ST_cvt_static_to_extern(*st)) {
        continue;
      }
   ...
   /* record global name of FSTATIC and ST_cvt_static_to_extern st */
   FOREACH_SYMBOL(GLOBAL_SYMTAB, st, st_idx)
   {
      if ((ST_sclass(st) != SCLASS_FSTATIC) && !ST_cvt_static_to_extern(*st)) {
        continue;
...
#endif
{
   ...
   //addad by wy
   W2C_Emit_Gsymtab = CLIST_emit_gsymtab;
...
W2C_Push_PU(const WN *pu, WN *body_part_of_interest)
{
   ...
   Stab_initialize();
   #ifdef W2C
   W2CF_Treety_Map = WN_MAP32_Create(&W2C_Parent_Pool);
   #endif
...
#ifdef W2C
   WN_MAP_Delete(W2CF_Treety_Map);
   W2CF_Treety_Map = WN_MAP_UNDEFINED;
#endif
			 ...
			 TYPE_ID     istore_mtype)
{
   ...
   is_bit_field = (istore_mtype == MTYPE_BS);
#endif
...
W2C_Outfile_Init(BOOL emit_global_decls)
{
   ...
   Write_String(W2C_File[W2C_DOTC_FILE], W2C_File[W2C_LOC_FILE],
		...
		"\n\n");
#ifdef W2C
   FILE *trace = fopen("w2c_normalize.t", "w+");
   #endif
...
W2C_Outfile_Translate_Pu(WN *pu, BOOL emit_global_decls)
{
   ...
   // add by mw , use global src_cplus to avoid negative effect 
   // on translation when src is C
   if (PU_cxx_lang(Pu_Table[ST_pu(WN_st(pu))])) {
     src_cplus = TRUE;
   } else {
     src_cplus = FALSE;
   }
...
#if 1 // simd whirl2c
   lower_actions = LOWER_IO_STATEMENT | LOWER_INTRINSIC | LOWER_INLINE_INTRINSIC;
#endif
   ...
   Set_Error_Phase ("WHIRL To C");
   if (emit_global_decls) {
     /* Include <stdarg.h> in the .h file if va_arg is used*/
     // gcc004 modified by shihui, if vararg is used, include
     // these head file.
     extern BOOL   whirl2c_include_stdarg;
     extern BOOL   whirl2c_include_stdarg_added;
     if(whirl2c_include_stdarg && !whirl2c_include_stdarg_added) {
		   Write_String(W2C_File[W2C_DOTH_FILE], NULL/* No srcpos map */,
		   "/* Include builtin types and operators */\n"
		   "#include <stdarg.h>\n\n");
		   whirl2c_include_stdarg_added = TRUE;
		 }
		 WN2C_translate_structured_types();
   }
   ...
   // normalize pu 
   if (!CLIST_emit_gsymtab && CLIST_normalize) {
     Normalize_Rec(pu, pu);
   }
   if (!CLIST_emit_gsymtab && CLIST_normalize && Get_Trace(TKIND_IR, TP_WHIRL2FC))
   {
     fputs(DBar, TFile);
     fprintf(TFile, "After Whirl Normalize: \n");
     fdump_tree(TFile, pu);
   ...
   // delete Lno_Opt_Map
   extern WN_MAP Lno_Opt_Map;
   if (Lno_Opt_Map != WN_MAP_UNDEFINED) {
     WN_MAP_Delete(Lno_Opt_Map);
     Lno_Opt_Map = WN_MAP_UNDEFINED;
   }
...
W2C_Outfile_Fini(BOOL emit_global_decls)
{
   ...
   if (Get_Trace(TKIND_IR, TP_WHIRL2FC))
   {
     fputs(DBar, TFile);
     fprintf(TFile, "Global symtab After Normalize: \n");
...
static void
lower_multiarray (WN* parent, WN* wn)
{
  Is_True (WN_operator (wn) == OPR_ARRAY, ("Not array opr"));

#if 0
  extern void dump_tree(WN*);
  dump_tree (wn);
#endif
  ...
  WN_kid(parent, kid) = tmp;
}


extern void
W2C_lower_array (WN* wn)
{

  if (WN_operator (wn) == OPR_BLOCK) {
    for (WN* wni = WN_first(wn); wni != NULL; wni = WN_next (wni))
      W2C_lower_array (wni);
  }
  else {
    for (INT32 i = 0; i < WN_kid_count (wn); i++) {
      WN* wni = WN_kid (wn, i);
      if (WN_operator (wni) == OPR_ARRAY &&
	  WN_num_dim(wni) >= 2)
	lower_multiarray (wn, wni);
      else
	W2C_lower_array (wni);
    }
  }
