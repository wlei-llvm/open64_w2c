...
#ifdef W2C
static void
W2CF_Get_Basename(const char *original_name, char *basename, INT32 *sym_id, mBOOL is_global=FALSE)
{
...
#define MAX_NUMERIC_SUFFIX_SIZE 8 /* fits into a UINT32 */
   ...
   for (name_size = 0; valid_name[name_size] != '\0'; name_size++)
    ...
    if (isdigit(valid_name[name_size - 1])) {
      basename[name_size++] = 't';
      basename[name_size++] = 't';
      basename[name_size++] = 't';
      basename[name_size] = '\0';
    }
#else

   // gcc009 modified by shihui
   // if it is gloabl variable no need to append ttt.
   #if 0
   if (isdigit(valid_name[name_size - 1]) && !is_global) {
     basename[name_size++] = 't';
     basename[name_size++] = 't';
     basename[name_size++] = 't';
     basename[name_size] = '\0';
...
#ifdef BUILD_WHIRL2F
   if(strcmp(basename,"MAX")==0 || strcmp(basename,"MIN")==0){
     basename[++name_size] = 't';
     basename[++name_size] = 't';
     basename[++name_size] = 't';
     basename[++name_size] = '\0';
...
W2CF_Identical_Symkinds(W2CF_SYMBOL *sym1, W2CF_SYMBOL *sym2)
{
   ...
   if (identical/*kinds*/)
   {
	 ...
	 #else
	 	 identical = TY_IDX_index(W2CF_SYMBOL_ty(sym1)) == 
                       TY_IDX_index(W2CF_SYMBOL_ty(sym2));
      #endif
                   ...
                   const char *valid_name = NULL)
{
    ...
    * changing the symid if it is already in use or is invalid.
    */
   W2CF_Insert_Symbol(symhdr, symbol, is_global);

   /* The symbol name will be constructed from the basename and the
    * the symbol identifier.
    */
   #ifdef BUILD_WHIRL2C
   // modified by shihui for gap's mutli defintion of extern varialbe
   // in different scope
   #if 0
   if(is_global) {
     W2CF_SYMBOL_name(symbol) = W2CF_Symtab_Alloc_Chars(symtab, strlen(valid_name)+1);
     (void)strcpy(W2CF_SYMBOL_name_string(symtab, symbol), valid_name);
   }
   else
   #endif
   #endif
   ...
   {
     symname = strdup(W2CF_SYMHDR_basename_string(symtab, symhdr));
...
W2CF_Symtab_Nameof_St(const ST *st)
{
   ...
   mBOOL is_global = FALSE;
#ifdef BUILD_WHIRL2C
   // gcc009 modified by shihui , if it is not an auto varialbe 
   // and pass this info into W2CF_Get_Basename
   ...
   #if 0
   if(LNO_IPA_Enabled){
     //modified  by wy,to rename the iterance variable with the offset attribute 

     if( strncmp(valid_name,"__ctype_b",9) != 0 &&
	 	strncmp(valid_name,"environ",7) != 0 &&
	 	strncmp(valid_name,"args",4) != 0 &&
	 	strncmp(valid_name,"argv",4) != 0&&
	 	strncmp(valid_name,"argc",4) != 0 &&
	 	strncmp(valid_name,"env",3) != 0 &&
	 	strncmp(valid_name,"anon",4) == 0 &&
                is_global) {
       Is_True(ST_class(st) != CLASS_PREG, ("St class should not be CLASS_PREG"));
       UINT offset_suffix = ST_st_idx(st);
       UINT valid_name_long = strlen(valid_name);
       UINT char_offset_suffix_long = 0;
       char *mid_name;
       if(offset_suffix != 0){
         while(offset_suffix/10 != 0 || offset_suffix%10 !=0){
            offset_suffix = offset_suffix/10;
            char_offset_suffix_long++;
         }
       }else{
         char_offset_suffix_long = 1 ;
       } 
       mid_name = Get_Name_Buf_Slot(valid_name_long+char_offset_suffix_long+2);
       sprintf(mid_name, "%s_%d", valid_name, ST_st_idx(st));
       valid_name = mid_name;   
     }
   ...
   symname = Get_Name_Buf_Slot(strlen(valid_name) + 32);
   W2CF_Get_Basename(valid_name, symname, &symid, is_global);
   ...
   /* FIXME: UPC & Open64@Rice */
   //return valid_name;

   /* Return the resultant disambiguated name */

#ifdef BUILD_WHIRL2C
   // don't change the name of global variable.
   // but global variable name may be conflict with local variable, so global variable name
   // should be insert into W2CF symtab.
   if (LNO_IPA_Enabled) {
     if (is_global && (ST_sclass(st) != SCLASS_FSTATIC) && 
	   !ST_cvt_static_to_extern(*st)) {
       return valid_name;
     } else {
       return W2CF_SYMBOL_name_string(symtab, symbol);
     }
...
W2CF_Symtab_Nameof_Ty(TY_IDX ty)
{
   ...
   if(LNO_IPA_Enabled){
     ...
     ty_idx_rename = Get_Name_Buf_Slot(strlen(TY_name(ty))+char_ty_idx_suffix_long+2) ;
     sprintf(ty_idx_rename, "%s_%d",TY_name(ty),Ty_Table[ty].u1.tylist);
     valid_name=ty_idx_rename;
...
#ifdef BUILD_WHIRL2C
  // add by mw, cxx2c producing struct exception have
  // name conflict with that of math.h. In normal case
  // cxx compiler will add a __cplusplus macor, but the 
  // macro is inconsistent with c frontend, I have to 
  // change the name exception to exception_cc to 
  // walkaround it. leave aside it for improvement
  // eon bug8
  extern BOOL src_cplus;
  if (src_cplus && 
      TY_kind(ty) == KIND_STRUCT &&
  	strcmp(valid_name, "exception") == 0) {
    return strdup("exception_cc");
  }
  if (strcmp(W2CF_SYMBOL_name_string(symtab, symbol), "exception") == 0) {
    return strdup("exception_cc");
  }
#endif
#ifndef W2C
...
W2CF_Symtab_Nameof_Preg(const TY_IDX preg_ty, PREG_NUM preg_num)
{
   ...
   if (valid_name == NULL || valid_name[0] == '\0')
   {
    ...
    //by yxli
   //diff st in common from st of preg
#ifdef BUILD_WHIRL2F
    INT32 i= strlen(valid_name);
    const char  *valid_name2 = Concat2_Strings(valid_name,"_preg");
    symname = Get_Name_Buf_Slot(strlen(valid_name2) + 32);
    W2CF_Get_Basename(valid_name2, symname, &symid);
#endif
// add by mw
#ifdef BUILD_WHIRL2C    
   const char  *valid_name2 = Concat2_Strings(valid_name,"_preg");
