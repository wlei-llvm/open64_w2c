...
static const char *const C_Reserved_St_Name[] =
{
   ...
   "strrchr",
   "memcmp",
   "__builtin_constant_p" , ///wanglei for debug ??? is it the best 
   "strspn",
   "strcat",
   "strtok"
   "strerror",
...
#ifdef BUILD_WHIRL2C
    // add by mw, if param ty is a const pointer and
    // prototype is a struct needed to be converted to a pointer 
    // pointed to struct, give the prototype ty a flag it needs a 
    // a const qualifier when output func declaration. eon bug13
    W2FC_TY_POINTER_NEEDS_CONST = 0x04, 
    // when the param type is not identical to the prototype
    // we should convert struct to pointer pointed to struct
    W2FC_TY_SHOULD_CONVERT = 0x08,
    // before output func(struct ty *, ...),  we need a declaration of 
    // struct ty: struct ty;
    W2FC_NEEDS_NO_TY_DECL = 0x10,
#endif
...
#ifdef BUILD_WHIRL2C
extern void 
Set_TY_should_convert(const TY_IDX ty) 
{
  W2fc_ty_tab->Set_w2fc_flag(TY_IDX_index(ty),W2FC_TY_SHOULD_CONVERT) ;
}

extern void 
Reset_TY_should_convert(const TY_IDX ty) 
{
  W2fc_ty_tab->Reset_w2fc_flag(TY_IDX_index(ty),W2FC_TY_SHOULD_CONVERT) ;
} 

extern BOOL 
TY_should_convert(const TY_IDX ty) 
{
  return (W2fc_ty_tab->Check_w2fc_flag(TY_IDX_index(ty),W2FC_TY_SHOULD_CONVERT)) ;
}
...
extern void 
Set_TY_needs_const(const TY_IDX ty) 
{
  W2fc_ty_tab->Set_w2fc_flag(TY_IDX_index(ty),
  	W2FC_TY_POINTER_NEEDS_CONST) ;
}

extern void 
Reset_TY_needs_const(const TY_IDX ty) 
{
  W2fc_ty_tab->Reset_w2fc_flag(TY_IDX_index(ty),
  	W2FC_TY_POINTER_NEEDS_CONST) ;
} 

extern BOOL 
TY_needs_const(const TY_IDX ty) 
{
  return (W2fc_ty_tab->Check_w2fc_flag(TY_IDX_index(ty),
  	W2FC_TY_POINTER_NEEDS_CONST)) ;
}

extern void 
Set_TY_needs_no_ty_decl(const TY_IDX ty) 
{
  W2fc_ty_tab->Set_w2fc_flag(TY_IDX_index(ty),
  	W2FC_NEEDS_NO_TY_DECL) ;
}

extern void 
Reset_TY_needs_no_ty_decl(const TY_IDX ty) 
{
  W2fc_ty_tab->Reset_w2fc_flag(TY_IDX_index(ty),
  	W2FC_NEEDS_NO_TY_DECL) ;
} 
...
extern BOOL 
TY_needs_no_ty_decl(const TY_IDX ty) 
{
  return (W2fc_ty_tab->Check_w2fc_flag(TY_IDX_index(ty),
  	W2FC_NEEDS_NO_TY_DECL)) ;
}
...
#else
    // add by mw , ty table will grow in whirl2c
  W2fc_ty_tab = CXX_NEW(W2FC_FLAG_ARRAY(2 * TY_Table_Size()),Malloc_Mem_Pool);
#endif
