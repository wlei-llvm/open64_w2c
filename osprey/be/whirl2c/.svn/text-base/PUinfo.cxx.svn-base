...
typedef DYN_ARRAY<char*> INDEX_INTRINSIC_PREG;
extern INDEX_INTRINSIC_PREG* index_intrinsic_preg;
#endif
//end 2006-10-9
...
//by yxli
//for extern file reference
PREG_INFO *
Get_Preg_Info2(INT16 preg_num)
{
   PREG_INFO *preg_info = NULL;
...
#ifdef BUILD_WHIRL2F
       PREG_INFO_array_len(preg_info)=0;//by yxli 2006-8-18
#endif 
...
Accumulate_Expr_PUinfo(WN *root)
{
	...
	wn_iter = PUINFO_WN_WALK_TreeNext(wn_iter))
   {
	 ...
	 case OPR_RETURN:
        ...
        //by yxli 2006-10-9
        //pregA=loc(A) pregB=loc(B) ==> index(pregA,pregB)
        //e.g. getirv.f90 in fma3d
        #ifdef BUILD_WHIRL2F
         case OPR_INTRINSIC_OP:
            if(strcmp(WN_intrinsic_name((INTRINSIC) WN_intrinsic(wn)),"F90INDEX")==0){
               for(INT32 i=0;i<=2;i=i+2){
                  TY_IDX opnd_type = WN_Tree_Type(WN_kid(wn, i));
                  if(!TY_Is_Character_Reference(opnd_type) && 
                     !TY_Is_Chararray_Reference(opnd_type)){
                      WN* kid=WN_kid0(WN_kid(wn,i));
                      if(WN_operator(kid)==OPR_LDID &&
                         ST_class(WN_st(kid))==CLASS_PREG){
                         char* preg_name;
                         INT32 j;
                         TY_IDX preg_ty = PUinfo_Preg_Type(ST_type(WN_st(kid)), WN_load_offset(kid));
                         preg_name = (char*)W2CF_Symtab_Nameof_Preg(preg_ty,WN_load_offset(kid));
                         for(j=0;j<(*index_intrinsic_preg).Elements();j++){
                            if(strcmp(preg_name,(*index_intrinsic_preg)[j])==0) 
                              break;
                         }
		         if(j==(*index_intrinsic_preg).Elements())
                           index_intrinsic_preg->AddElement(preg_name);
                      }
