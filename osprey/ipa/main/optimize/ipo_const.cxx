...

{
   ...
   if (param_count == WN_num_formals (w))
    ...
    ALIGN_DYN_ARRAY *formal_annot = NULL;                                                   //cmchen 08-3-4          
    ...
    if (IPA_Enable_Align_Analysis)

      formal_annot = n->Align_Annot();                                             

    INT k=0;

 

     //modified by wy,add TY entry for the function propagated

    TY_IDX new_idx;

    TY &new_ty = New_TY (new_idx);

    TY_IDX func_ty_idx=Ty_Table[ST_type(WN_st(func_node))].u1.tylist;

    TY_IDX ret_ty_idx=TY_ret_type(ST_pu_type(WN_st(func_node)));

    TYLIST tylist_idx;

    TY_Init (new_ty, 0, KIND_FUNCTION, MTYPE_UNKNOWN, 0);

    if(TY_is_varargs(ST_type(WN_st(w))))

      Set_TY_is_varargs(new_ty);

    Set_TY_mtype(new_ty,TY_mtype(ST_pu_type(WN_st(func_node))));

    Set_TYLIST_type (New_TYLIST (tylist_idx), ret_ty_idx);

    Set_TY_tylist (new_ty, tylist_idx);                                                           // cmchen 08-3-4   1165-1166  1169-1180               
    ...
    for (i = 0; i < WN_num_formals(w); i++) {
         ...
         if ((*cprop_annot)[i].Is_remove_param ()) {
           ...
           if (IPA_Enable_Align_Analysis) {
             ...
             ACTUAL_ALIGN_INFO *align_info = NULL;

             if (formal_annot != NULL) {

               align_info = &(*formal_annot)[i];

               align_info->Set_is_formal_removed();

             }

           }                                                                                                       // cmchen 08-3-4   1193-1199

         } else {

             WN_kid(func_node,k) = id;

           Set_TYLIST_type (New_TYLIST (tylist_idx), Tylist_Table[i+TY_parms(ST_pu_type(WN_st(func_node)))]);

            if(!k)

               Set_TY_has_prototype(new_idx);                                                 //cmchen 08-3-4  1202-1204
    ...
    Pu_Table[WN_st(func_node)->u2.pu].prototype=new_idx;                         // cmchen 08-3-4 1209-1210
...
IPA_Propagate_Constants (IPA_NODE* n, BOOL delete_const_param)
{
    ...
    VALUE_DYN_ARRAY* const_formal_array = NULL; 
#ifdef W2C
    ALIGN_DYN_ARRAY *formal_annot = NULL;   
   // TY_IDX new_idx=0;
#endif
    if (IPA_Enable_Array_Sections) {
	IPA_NODE_SECTION_INFO* sec = n->Section_Annot();
	...
	//new_idx=Tylist_Table[tylist_idx;
   ...
   // error
  //         Set_TYLIST_type (New_TYLIST (tylist_idx), Tylist_Table[i+TY_parms(ST_pu_type(WN_st(func_node)))]);
     //      Set_TYLIST_type (New_TYLIST (tylist_idx), Tylist_Table[i+from_idx]);
	//   if(!k)
	//      Set_TY_has_prototype(new_idx);    		
#endif	   
	    ++k;
#ifdef KEY
	    Set_TYLIST_type (New_TYLIST (tylist_idx), Tylist_Table[from_idx]);
   ...
   // Pu_Table[WN_st(func_node)->u2.pu].prototype=new_idx;  
#endif
    if (n->Has_Aliased_Formal ()) {
	// there are STs that are based on the deleted formals, so we
	// need to change their storage_class to SCLASS_AUTO
