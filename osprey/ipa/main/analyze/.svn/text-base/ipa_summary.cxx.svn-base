...
#ifdef W2C
//-------------------------------------------------------------------
// here are funcs used in ip align analysis
//-------------------------------------------------------------------
ACTUAL_ALIGN_INFO*
IPA_get_align_info (const IPA_NODE* node)
{
  IP_FILE_HDR& hdr = node->File_Header();
  SUMMARY_FILE_HEADER *summary_header = IP_FILE_HDR_file_header(hdr);
  if (summary_header->Get_align_info_size()) {
    return (ACTUAL_ALIGN_INFO *) (IP_FILE_HDR_summary (hdr) 
		+ summary_header->Get_align_info_offset()); 
  } 
}

ACTUAL_ACCESS_ARRAY*
IPA_get_access_array (const IPA_NODE* node)
{
  IP_FILE_HDR& hdr = node->File_Header();
  SUMMARY_FILE_HEADER *summary_header = IP_FILE_HDR_file_header(hdr);
  if (summary_header->Get_access_array_size()) {
    return (ACTUAL_ACCESS_ARRAY *) (IP_FILE_HDR_summary (hdr) 
		+ summary_header->Get_access_array_offset());  
  }
}

ACTUAL_ACCESS_VECTOR*
IPA_get_access_vector (const IPA_NODE* node)
{
  IP_FILE_HDR& hdr = node->File_Header();
  SUMMARY_FILE_HEADER *summary_header = IP_FILE_HDR_file_header(hdr);
  if (summary_header->Get_access_vector_size()) {
    return (ACTUAL_ACCESS_VECTOR *) (IP_FILE_HDR_summary (hdr) 
		+ summary_header->Get_access_vector_offset());  
  }
}

ALIGN_INTSYMB_NODE*
IPA_get_intsymb_node (const IPA_NODE* node)
{
  IP_FILE_HDR& hdr = node->File_Header();
  SUMMARY_FILE_HEADER *summary_header = IP_FILE_HDR_file_header(hdr);
  if (summary_header->Get_intsymb_node_size()) {
    return (ALIGN_INTSYMB_NODE *) (IP_FILE_HDR_summary (hdr) 
		+ summary_header->Get_intsymb_node_offset());  
  }
}
  
ALIGN_SUMPROD_NODE*
IPA_get_sumprod_node (const IPA_NODE* node)
{
  IP_FILE_HDR& hdr = node->File_Header();
  SUMMARY_FILE_HEADER *summary_header = IP_FILE_HDR_file_header(hdr);
  if (summary_header->Get_sumprod_node_size()) {
    return (ALIGN_SUMPROD_NODE *) (IP_FILE_HDR_summary (hdr) 
		+ summary_header->Get_sumprod_node_offset());  
  }
}  
#endif
