...
#ifdef W2C
 ALIGN_DYN_ARRAY*       _align_annot;  // align analysis annotation 
#endif
...
#endif // _LIGHTWEIGHT_INLINER
#ifdef PACE 
  COMP_UNIT   *_comp_unit;     //contains GSA and IPSSA Form
  BOOL         _self_recur;    // whether NODE is self-recursive
  UINT32       _topo_id;      // topological order, start from 1
  UINT32       _rtopo_id;     // reverse topological order, start from 1
#endif
...
public:
...
#ifdef W2C
  void Set_Align_Annot(ALIGN_DYN_ARRAY* annot) { _align_annot = annot; }
  ALIGN_DYN_ARRAY* Align_Annot() const { return _align_annot; } 
#endif
  ...
  BOOL        Self_recur() const { return  _self_recur; } 
  BOOL        Set_self_recur(BOOL b) { _self_recur = b; } 
  UINT32      Topo_id() const { return _topo_id; }
  void        Set_topo_id( UINT32 i) { _topo_id = i ;} 
  UINT32      Rtopo_id() const { return  _rtopo_id; } 
  void        Set_rtopo_id( UINT32 i) { _rtopo_id = i ;} 
  PU_IDX      Pu_idx () const
  {
    Is_True(_func_st != 0, ("IPA_NODE: null ST pointer"));
    Is_True(ST_pu(_func_st) > 0 && ST_pu(_func_st) < Pu_Table.size(),
...
#ifdef PACE
  MEM_POOL*      Mem_pool() const { return  _pool; }
#endif
