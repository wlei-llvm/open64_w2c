...
namespace
{
    ...
    // add by mw
    struct TY_EQUAL_IGNORE_NAME {
        BOOL operator() (const TY& ty1, const TY& ty2) const {
            TY tmp_ty1 = ty1;
            TY tmp_ty2 = ty2;
            Set_TY_name_idx(tmp_ty1, 0);
            Set_TY_name_idx(tmp_ty2, 0);
            BOOL equal = (tmp_ty1 == tmp_ty2);
            return equal;
        }
    };
#endif
...
#ifdef W2C
// add by mw
typedef hashtable<TY_IDX, TY, TY_HASH_IGNORE_NAME, TY_EXTRACT_KEY, 
                  TY_EQUAL_IGNORE_NAME,
		  mempool_allocator<TY_IDX> > ANONYMOUS_TY_HASH_TABLE;
#endif
...
#ifdef W2C
static ANONYMOUS_TY_HASH_TABLE* anonymous_ty_hash_table;
#endif
...
#ifdef W2C	
    anonymous_ty_hash_table = CXX_NEW (ANONYMOUS_TY_HASH_TABLE (1000, 
                                            TY_HASH_IGNORE_NAME (), 
                                            TY_EQUAL_IGNORE_NAME (), 
					    TY_EXTRACT_KEY (), pool),
			     pool);	
...
#ifdef W2C
	anonymous_ty_hash_table->find_or_insert (make_TY_IDX (i));
#endif
    }
...
#ifdef W2C
TY_IDX
Insert_Unique_Ty (const TY& ty, char *tyname)
{
    TY new_ty = ty;

    Setup_Ty (new_ty);

    // add by mw, for remove duplicating ty items in symtab.I/G
    // when ipa is on
    // equal_to<> in template params of TY_HASH_TABLE will compare
    // every item in ty. so ty with different name: .anonymous.1
    // and .anonymous.2 will be treated as different ty, but when
    // the ty name is .anonymous.xxx, we want to use other items 
    // in ty to judge them different or not, so a new ty hash table
    // anonymous_ty_hash_table and new equal func TY_EQUAL_IGNORE_NAME
    // are used
    TY_IDX *resultpt = NULL;
    STR_IDX saveidx = 0;
    BOOL anonymous = FALSE;
    ty_to_be_inserted = &new_ty;
    if (tyname != NULL && strncmp(tyname, ".anonymous.", 11) == 0) {
      anonymous = TRUE;
      resultpt = &(anonymous_ty_hash_table->find_or_insert ((TY_IDX) -1));
      if ((*resultpt) != (TY_IDX) -1) {
        return (*resultpt); 
      }
    }
...
Insert_Recursive_Type (TY_IDX ty_idx)
{
    ...
    case KIND_STRUCT:
#ifdef W2C		
    //add by mw: array should be inserted into recursive table to remove duplicate entry in ty table
    case KIND_ARRAY:
#endif
    case KIND_FUNCTION:
