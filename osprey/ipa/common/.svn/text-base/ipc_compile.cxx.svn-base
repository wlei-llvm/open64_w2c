                            ...
                            const PU_Info* pu, UINT32 ProMP_id)
{
...
#ifdef W2C
    sprintf(str, " -show  -TENV:ipa_ident=%ld -TENV:read_global_data=%s %s",
          time(0),
          whirl_symtab_name,
          IPA_Enable_AutoGnum?"-Gspace 0":"");
#else
  ...
  for (i = argv.begin(); i != argv.end(); ++i) {
#ifdef W2C
    if (strstr(*i, "-PHASE") != NULL && 
          strstr(*i, "list") != NULL) {
      if (len == 0) {
        len += strlen(*i);
        w2cfarg = (char *)malloc(len+1);
        strcpy(w2cfarg, *i);
      } else {
        len += strlen(*i);
        w2cfarg = (char *)realloc(w2cfarg, len+2);
        strcat(w2cfarg, " ");
        strcat(w2cfarg, *i);
      }
    }
...
void ipacom_doit (const char* ipaa_filename)
{
  ...
  if (strlen(elf_symtab_name) != 0) {
    ...
    if (Feedback_Filename) {
      ...
      char *W2c_gsymtab = "";
      char *w2cipa = "";
      if (strstr(w2cfarg, "clist")) {
        W2c_gsymtab = "-CLIST:emit_gsymtab";
        w2cipa = "-LNO:ipa";
      }
      fprintf(makefile, " %s %s %s\n\n", W2c_gsymtab, w2cfarg, w2cipa);
#else
      fprintf(makefile, "\n\n");
#endif
	} else if (Annotation_Filename) {
