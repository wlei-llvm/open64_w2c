...
#ifdef W2C
	   || (strcmp(&Str_Table[TY_name_idx (merged_ty)], 
                	&file_tables->symstr_tab[TY_name_idx (new_ty)]) != 0 &&
	      	// at least one name != .anonymous. or it is deem to be identical
	     	(strncmp(TY_name(merged_ty), ".anonymous.", 11) != 0 ||
              	strncmp(&file_tables->symstr_tab[TY_name_idx (new_ty)], ".anonymous.", 11) != 0))
#endif
	)
...
namespace
{
	...
	if (TY_Inserted (my_mapped_idx)) {
	    ...
	    return my_mapped_idx;
            ...
            if (IPA_Enable_Old_Type_Merge) {
#ifdef W2C
              // add by mw, transfer tyname to Insert_Unique_Ty,
              // In func Insert_Unique_Ty, we cannot get file_tables
              char *tyname = &file_tables->symstr_tab[TY_name_idx(ty)]; 
              return Insert_Unique_Ty (ty, tyname);
#else			
