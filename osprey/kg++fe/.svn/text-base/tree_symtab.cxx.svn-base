 ...
 */
TY_IDX
Get_Func_TY (tree type_tree, tree parm_list)
{
  ...
  Is_True(TREE_CODE(type_tree) == FUNCTION_TYPE ||
          TREE_CODE(type_tree) == METHOD_TYPE, 
  ...
  for (arg = parm_list; Tylist_Table[parms] != TY_IDX_ZERO && arg;
       arg = TREE_CHAIN(arg), parms = TYLIST_next(parms)) {
    TY_IDX arg_ty = Get_TY(DECL_ARG_TYPE(arg));
    TY_IDX proto_ty = TYLIST_item(Tylist_Table[parms]);
...
#ifdef W2C
{
                ...
                tree type = TREE_TYPE(type_tree);
                if (TREE_CODE(type) == FUNCTION_TYPE ||
                    TREE_CODE(type) == METHOD_TYPE) 
                  idx = Make_Pointer_Type (Get_Func_TY (TREE_TYPE(type_tree), NULL));
                else 
...
#ifdef W2C
        TY_IDX func_ty_idx = Get_Func_TY(TREE_TYPE(decl_node), 
                                         DECL_ARGUMENTS(decl_node));   
#else
